...
The fingerprint configuration allows you to include additional sources and enable debug mode:
...
export default defineConfig({
  ...
  fingerprint: {
    extraSources: ["resources/**", ".gitignore", "./git-submodule"], // Additional sources to include in fingerprint
    debug: true, // Enable debug mode for detailed fingerprint information
  },
});
```
