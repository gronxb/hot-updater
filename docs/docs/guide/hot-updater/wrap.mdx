...
When a force update bundle is downloaded, the app will automatically reload. If `false`, `isForceUpdate` will be returned as `true` in `onUpdateProcessCompleted` but the app won't reload. default is `true`.
...
When a force update exists, the app will not reload. `isForceUpdate` will be returned as `true` in `onUpdateProcessCompleted`.
...
export default HotUpdater.wrap({
  ...
  onUpdateProcessCompleted: ({ status, isForceUpdate, id }) => {  // [!code hl]
    console.log("Bundle updated:", status, isForceUpdate, id);  // [!code hl]
    if (isForceUpdate) { // [!code hl]
