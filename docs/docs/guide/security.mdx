  ...
  plugins: [
    ...
    [
      'module:react-native-dotenv',
      {
        envName: 'APP_ENV',
        moduleName: '@env',
        allowlist: ['HOT_UPDATER_SUPABASE_URL'], // ✅ Only allow tokens that are safe to expose
        //  allowlist: ['HOT_UPDATER_SUPABASE_ANON_KEY', 'HOT_UPDATER_CLOUDFLARE_API_TOKEN'], ❌ Never allow private tokens
        path: '.env',
      },
    ],
  ],
