...
Schema `1.0.0` adds Releases, Release catalogs, ordered compiler indexes, and
atomic revision/generation checks. It removes the legacy desired-state RPCs.
Managed Supabase deployments track applied changes through Supabase migration
history; they do not write or consume the generic server migrator's
`schema.core` setting. Keep existing migration files immutable, and back up the
database before applying any pending provider migration.

Apply all pending Supabase migrations before upgrading the CLI writer or
redeploying the Edge Function. Patch-bearing bundle inserts and patch
replacements use service-role-only database functions so each bundle mutation
commits atomically; if those functions are not installed, the operation fails
instead of falling back to sequential writes.
...
export default HotUpdater.wrap({
  baseURL: "https://updates.example.com/hot-updater", // [!code ++]
