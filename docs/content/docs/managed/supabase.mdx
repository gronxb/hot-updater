...
Supabase's direct Edge Function URL is the correctness-only mode. It does not
provide a documented reusable response cache in front of Function execution,
so it must not be used as evidence for the shared-cache cost profile.

For that profile, configure an HTTPS CDN that proxies the complete Edge
Function base URL and set:

```dotenv
HOT_UPDATER_SUPABASE_CATALOG_CDN_URL=https://updates.example.com/hot-updater
```

The CDN cache key must include host, canonical path, `x-api-key`, and supported
content encoding, and exclude installation state. It must honor the catalog's
five-second shared TTL and must not cache private/no-store responses. Init uses
this URL in the generated React Native configuration. Without it, init warns
and `hot-updater doctor --server-base-url <direct-edge-url>` reports that the
endpoint is not stress-safe.
