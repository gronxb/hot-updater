...
Docker or OrbStack is needed only for a local Supabase stack. It is not required for the managed setup in this guide.
...
1. **Select a Supabase Organization**: Choose an existing organization. If your account has none, create one in the Supabase Dashboard before running init.
2. **Create or Select a Project**: Choose an existing project or create a new one within the selected organization.
3. **Configure Storage**: Select an existing bucket or create a new private bucket. If the selected bucket is public, init asks for confirmation before making it private.
4. **Database Migration**: Apply the Hot Updater schema to the selected project.
