...
<Callout type="info">
The managed Supabase runtime exposes the public update-check endpoints and `/version`. Internally, it is equivalent to `createHotUpdater({ features: { updateCheck: true, bundles: false } })`, so bundle management endpoints are not mounted on the public function URL.
</Callout>
