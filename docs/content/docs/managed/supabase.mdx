...
SHA-256 digest is embedded in the Edge Function runtime. To rotate a known key
or recover a lost one, replace the local value with a new canonical 32-byte
...
response reports `eventIngestion: true` and `analyticsQueries: true` because the
database supports Analytics. The Supabase platform health endpoint `/ping`
