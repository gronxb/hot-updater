...
`bundle_patches` schema. Managed Supabase deployments track applied changes
through Supabase migration history; they do not write or consume the generic
server migrator's `schema.core` setting. Keep existing migration files
immutable, and back up the database before applying any pending provider
migration.
