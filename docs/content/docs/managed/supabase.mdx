...
Supabase service-role key, administrator secret, or other infrastructure
credential.
...
SHA-256 digest is embedded in the Edge Function runtime. The runtime uses that
digest to create an ephemeral in-memory managed Better Auth API-key session; it does not
add Better Auth tables to Postgres or store the raw key in the runtime. To
rotate a known key or recover a lost one, replace the local value with a new
...
Supabase service-role key or any other infrastructure setting from
...
Back up the database before applying the database v2 schema. The `0.38`
migration adds `bundle_events` and its Row Level Security policy. It is
forward-only; Hot Updater does not publish a down migration. Do not replay the
migration SQL manually after it succeeds. A rerun fails explicitly when the
...
const baseURL = "https://<project-id>.supabase.co/functions/v1/update-server";
...
<Callout type="info">
  The managed Supabase runtime mounts 12 Hot Updater routes: `/version`, four
  ...
  the database supports Analytics. The Supabase platform health endpoint `/ping`
...
Supabase rate limits, quotas, logging, and retention for your deployment. See
