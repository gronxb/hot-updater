...
<Callout type="info">
The managed Supabase runtime exposes the public update-check endpoints,
...
`eventIngestion: true` and `analyticsQueries: false`, independently from the
...
identifiers as untrusted telemetry, and configure Supabase rate limits, quotas,
logging, and retention for your deployment. Customize the generated edge
function's `eventIngestion.authorize` policy when your app can provide an
