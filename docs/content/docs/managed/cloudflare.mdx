...
4. **Choose R2 Privacy**: Confirm whether the selected bucket should be private.
...
HOT_UPDATER_CLOUDFLARE_R2_PRIVATE=true # or false
HOT_UPDATER_CLOUDFLARE_API_TOKEN=your-api-token
HOT_UPDATER_CLOUDFLARE_WORKER_NAME=your-worker-name
```

The account, bucket, D1 ID and name, R2 privacy choice, and Worker name are
deterministic replay state. The D1 API token and R2 access credentials are
written only when you consent to saving credential inputs; otherwise, provide
them again for a future infrastructure update.
