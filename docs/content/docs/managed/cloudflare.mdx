...
Database Plugin V2 continues to use the existing `bundles` and
`bundle_patches` D1 schema. Managed Cloudflare deployments track applied
changes through Wrangler's D1 migration history; they do not write or consume
the generic server migrator's `schema.core` setting. Keep existing migration
files immutable, and back up D1 before applying any pending provider migration.
...
`features.analytics` with `routes: { updateCheck: true, bundles: false }`.
