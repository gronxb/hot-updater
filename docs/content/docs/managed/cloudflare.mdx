...
SHA-256 digest is deployed as a Worker variable. To rotate a known key or
recover a lost one, replace the local value with a new canonical 32-byte
...
response reports `eventIngestion: true` and `analyticsQueries: true` because D1
supports Analytics. JWT-signed R2 download URLs remain outside the Hot Updater
handler and do not require the API-key header.
</Callout>
