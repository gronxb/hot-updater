...
<Callout type="info">
The managed Cloudflare runtime exposes the public update-check endpoints and
`/version`. Management routes, Analytics query routes, and `POST /events` are
not mounted on the public worker URL by default. A valid anonymous event request
returns 404 before persistence. The version response therefore reports
`eventIngestion: false` and `analyticsQueries: false`, independently from the
D1 database's structural Analytics support.
</Callout>

Enabling `analytics: true` in the app does not override this server policy. To
collect events, deploy a custom runtime with an explicit
`eventIngestion.authorize` decision and provider-level rate limits or quotas.
Do not expose ingestion with `authorize: () => true`. See
[Analytics](/docs/guides/analytics) for route and retention requirements.
