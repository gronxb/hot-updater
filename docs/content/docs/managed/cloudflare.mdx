...
<Callout type="info">
The managed Cloudflare runtime exposes the public update-check endpoints and `/version`. Internally, it is equivalent to `createHotUpdater({ routes: { updateCheck: true, bundles: false, analytics: false } })`, so management and Analytics query endpoints are not mounted on the public worker URL.
</Callout>
