...
- **Cloudflare API Token**: Create an API Token at https://dash.cloudflare.com/{your-account-id}/api-tokens with **R2 Edit** & **D1 Edit** permissions. Add **API Tokens Write** if you want `init` to create R2 API credentials for you.
...
2. **Enter R2 & D1 API Token**: Input your API token with R2 and D1 permissions. If it also has API Tokens Write, `init` can create R2 API credentials automatically.
3. **Select R2 Bucket**: Choose an existing R2 bucket or create a new one
4. **Set Up R2 API Credentials**: Create new R2 API credentials or enter existing Access Key ID and Secret Access Key values
