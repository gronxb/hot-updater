...
<Callout type="info">
The managed Cloudflare runtime exposes the public update-check endpoints,
`POST /events`, and `/version`. Management and Analytics query routes are not
mounted on the public worker URL by default. The version response reports
`eventIngestion: true` and `analyticsQueries: false`, independently from the D1
database's structural Analytics support.
</Callout>

Enabling `analytics: true` in the app sends events to this public ingestion
route without additional server configuration. Treat installation and user
identifiers as untrusted telemetry, and configure Cloudflare rate limits,
quotas, logging, and retention for your deployment. Customize the generated
worker's `eventIngestion.authorize` policy when your app can provide an
end-user credential or attestation. See [Analytics](/docs/guides/analytics).
