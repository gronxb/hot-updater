...
`hot-updater init` also prints a React Native snippet with a `hutk_` telemetry
key. That plaintext key is shown once for `analytics.telemetryKey`; it is stored
in D1 only as a hash/suffix and is not written to env files or provider config.
...
export default HotUpdater.wrap({
  ...
  analytics: {
    telemetryKey: "hutk_publishable_project_telemetry_key", // [!code ++]
  },
...
<Callout type="info">
The managed Cloudflare runtime exposes the public update-check endpoints, `POST /api/notify-app-ready`, and `/version`. Runtime telemetry must use the `x-hot-updater-telemetry-key` header; deploy API keys and signing keys are separate credentials. Internally, it is equivalent to `createHotUpdater({ routes: { updateCheck: true, bundles: false } })`, so bundle management endpoints are not mounted on the public worker URL.
</Callout>
