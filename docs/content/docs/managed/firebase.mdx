...
<Callout type="info">
The managed Firebase runtime exposes the public update-check endpoints and
`/version`. Management routes, Analytics query routes, and `POST /events` are
not mounted on the public function URL by default. A valid anonymous event
request returns 404 before persistence. The version response therefore reports
`eventIngestion: false` and `analyticsQueries: false`, independently from
