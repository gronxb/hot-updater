...
## Step 5: Add Optional Build Plugin Configuration

Most projects do not need any additional Hot Updater bundler plugin. The only
exception is Expo projects that use DOM components with `use dom`.

<Tabs items={['Bare', 'Re.Pack', 'Expo']}>
  <Tab value="Bare">

No additional Hot Updater Babel plugin is required for React Native CLI
projects.

</Tab>
<Tab value="Re.Pack">

No additional Hot Updater Re.Pack plugin is required.
...
If your Expo app uses DOM components with `use dom`, customize your Babel
configuration and add `hot-updater/babel-plugin`:
...
<Callout>
If you do not use Expo DOM components, you can skip the Babel plugin.
</Callout>
