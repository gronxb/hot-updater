...
<video src={"/docs/get-started/init-firebase.mov"} controls autoPlay loop muted />
...
SHA-256 digest is deployed with the Firebase Function. To rotate a known key or
...
stays public and is not part of the `createHotUpdater` handler.
</Callout>
...
Skip this step if you are using bare or Rock.
</Callout>

<Callout type="info">
If you use Expo DOM components (`use dom`), see the [Expo DOM Components (`use dom`)](/docs/build-plugins/expo#expo-dom-components-use-dom) section for the additional Babel plugin setup.
</Callout>
