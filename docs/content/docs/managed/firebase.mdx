...
<Callout type="info">
The managed Firebase runtime only exposes update-check endpoints. Internally, it is equivalent to `createHotUpdater({ routes: { updateCheck: true, bundles: false } })`, so bundle management endpoints and `/version` are not mounted on the public function URL.
</Callout>
