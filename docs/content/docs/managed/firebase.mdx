...
export default HotUpdater.wrap({
  baseURL: "https://<your-firebase-function-url>/api/check-update", // [!code ++]
...
Enabling `analytics: true` in the app sends events to the authenticated
`/events` route without additional server configuration. `requestHeaders`
supplies the key to both update checks and Analytics ingestion. If you configure
an HTTP-backed Hot Updater client separately, use the same key through
`commonHeaders`:
