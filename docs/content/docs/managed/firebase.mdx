...
- **Google Cloud CLI**: [Install the CLI](https://cloud.google.com/sdk/docs/install) and run `gcloud auth login`.
- **Firebase Project**: Create a new project at [Firebase](https://firebase.google.com) if you don’t have one.
- **Firebase Storage**: Enable Storage so the project has its canonical default bucket. Storage setup may require a billing plan.
...
2. **Verify Storage**: Verify that the selected project has its canonical default Firebase Storage bucket. If it is missing, enable Firebase Storage and rerun init.
