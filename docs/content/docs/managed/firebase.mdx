...
<Callout type="info">
The managed Firebase runtime exposes the public update-check endpoints,
`POST /events`, and `/version`. Management and Analytics query routes are not
mounted on the public function URL by default. The version response reports
`eventIngestion: true` and `analyticsQueries: false`, independently from
Firestore's structural Analytics support.
</Callout>
...
identifiers as untrusted telemetry, and configure Firebase rate limits, quotas,
logging, and retention for your deployment. Customize the generated function's
`eventIngestion.authorize` policy when your app can provide an end-user
credential or attestation. See [Analytics](/docs/guides/analytics).
