...
4. **Enter AWS Configuration**: Provide a profile name for shared-profile or SSO mode, or credentials for account/access-key mode
...
`fromIni({ profile: process.env.HOT_UPDATER_AWS_PROFILE! })`. SSO is also
profile-based and uses `fromSSO({ profile: process.env.HOT_UPDATER_AWS_PROFILE! })`.
Only account/access-key mode writes inline access-key credentials.

## Step 4: Replace Account Access Keys (Optional)

By this point, `.env.hotupdater` has already been created by `hot-updater init`.

This step only applies if you selected **account/access key** during `init`.
If you selected **current AWS CLI/default session**, **shared AWS profile**, or **SSO**,
you can keep using the generated credential-provider setup and skip this section.

The setup key may have excessive permissions due to infrastructure provisioning.

To avoid issues, replace it with credentials scoped for ongoing deployment, including `AmazonS3FullAccess` and `cloudfront:CreateInvalidation`.
