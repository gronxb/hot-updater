...
<Callout type="info">
The managed AWS runtime exposes the public update-check endpoints and `/version`. Internally, it is equivalent to `createHotUpdater({ routes: { updateCheck: true, bundles: false, analytics: false } })`, so management and Analytics query endpoints are not mounted on the public function URL.
</Callout>
