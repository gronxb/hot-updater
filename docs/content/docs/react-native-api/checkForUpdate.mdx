...
| `channel`         | `string`                       | ❌        | Channel to check. The switch is persisted only after the returned update is applied. |
...
The update source is always the `baseURL` or `resolver` configured by
`HotUpdater.wrap()` or `HotUpdater.init()`; it cannot be overridden in
`checkForUpdate`.
...
| `shouldForceUpdate` | `boolean` | Whether the app should reload immediately after download. |
| `fileUrl` | `string \| null` | Bundle archive URL, or `null` for a rollback without an archive. |
| `fileHash` | `string \| null` | Bundle hash or signature used by native verification. |
...
Prefer the returned `updateBundle()` helper so the values from that update
check are applied together.

### Example Return Value

The JSON response provides the data fields; the client adds the callable
`updateBundle()` helper.
