...
With [`verifyOnAppReady`](/docs/concepts/automatic-rollback#errors-caught-by-an-error-boundary)
on, `init()` is what promotes a newly installed bundle, one frame after it is
called. Call it from an effect in your first screen, so that a render error
reported by an error boundary can roll the bundle back.
