...
The `onSignatureVerificationFailure` callback is invoked when bundle signature verification fails. This is a security-critical event indicating the bundle may have been tampered with or the public key is misconfigured.
...
| Property | Type | Description |
|----------|------|-------------|
| `bundleId` | string | The ID of the bundle that failed verification |
| `message` | string | Human-readable error message from native layer |
...
export default HotUpdater.wrap({
  ...
  onSignatureVerificationFailure: ({ bundleId, message, error }) => { // [!code hl:8]
    // Log to your analytics/security service
    console.error(`Security alert: Bundle ${bundleId} failed verification`);
    console.error(message);

    // Show warning to user
    Alert.alert(
      "Security Warning",
      "Update verification failed. Please update the app from the app store.",
      [{ text: "OK" }]
    );
  },
  onError: (error) => {
    // This is also called after onSignatureVerificationFailure
    console.error("Update error:", error);
  },
})(App);
```

> **Note**: The `onError` callback is always called after `onSignatureVerificationFailure` for signature failures, allowing you to handle all errors in one place while still having specific handling for security events.

For more information about bundle signing, see the [Bundle Signing Guide](/docs/guides/bundle-signing).
