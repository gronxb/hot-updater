...
The `onError` callback handles all errors during the update process, including signature verification failures.

### Handling Signature Verification Errors

Use the `isSignatureVerificationError` utility to detect signature failures:
...
export default HotUpdater.wrap({
  ...
  onError: (error) => { // [!code hl:18]
    ...
    if (isSignatureVerificationError(error)) {
      const { bundleId, message } = extractSignatureFailure(error, "unknown");
...
|----------|-------------|
| `isSignatureVerificationError(error)` | Returns `true` if the error is a signature verification failure |
| `extractSignatureFailure(error, bundleId)` | Extracts structured failure info (`bundleId`, `message`, `error`) |
