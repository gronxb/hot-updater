...
on, `wrap()` promotes a newly installed bundle one frame after its first commit.
With a `fallbackComponent`, that commit is the fallback, so a render error in
your app after the update check is not covered.
