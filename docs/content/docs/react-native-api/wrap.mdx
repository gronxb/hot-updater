...
To enable Analytics, create a client from
`@hot-updater/analytics/react-native` and call `recordAppReady(result)` from
this wrapper's `onNotifyAppReady` callback. See the
