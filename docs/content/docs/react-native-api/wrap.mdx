...
> **Important:** `HotUpdater.wrap` is **required** for all apps. It ensures proper crash detection and rollback functionality. All HotUpdater methods will throw an error if wrap is not used.

`HotUpdater.wrap` supports two usage patterns:

### Option 1: With Automatic Updates (Recommended)
...
Use this for apps with custom update logic. This only calls `notifyAppReady()` automatically, giving you full control over when and how updates are applied.
...
// Just wrap the component - notifyAppReady will be called automatically
export default HotUpdater.wrap(App); // [!code hl]
```

Then use `checkForUpdate`, `runUpdateProcess`, or `updateBundle` manually when needed:
