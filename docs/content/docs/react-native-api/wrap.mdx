...
| ----------------- | --------------------------------------- | ----------------------------------------------------------- |
| `status`          | `"CHECK_FOR_UPDATE" \| "UPDATING"`      | Current update phase                                        |
| `progress`        | `number`                                | Download progress from 0 to 1                               |
| `downloadedBytes` | `number \| undefined`                   | Bytes downloaded when native progress provides a byte count |
| `totalBytes`      | `number \| undefined`                   | Total expected bytes when known                             |
| `message`         | `string \| null`                        | Optional message from the update response                   |
| `artifactType`    | `"archive" \| "diff" \| null`           | Artifact currently being downloaded                         |
| `details`         | `HotUpdaterDiffProgressDetails \| null` | Per-file progress for a diff artifact; otherwise `null`     |
