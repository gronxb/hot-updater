...
To enable Analytics, keep this wrapper and call
`HotUpdater.init({ analytics: true })` once before exporting the wrapped app.
The `analytics` option is not accepted by `HotUpdater.wrap`. See the
[Analytics guide](/docs/guides/analytics) for the complete setup.
