...
Returns the file IDs that were marked as failed and rolled back. These match
**File ID** in the console's Advanced diagnostics.
...
- identify which file IDs failed in production
- send rollback events to analytics or error tracking tools

### clearCrashHistory()

Clears the crash history so a previously failed file ID can be retried again.
...
When a bundle fails startup, Hot Updater adds that exact file ID to crash history and blocks it from being applied again. This prevents repeated crash loops from the same broken deployment.

To deploy a fixed version:

1. Fix the bug in your code
2. Deploy again with a new file ID
3. The new file ID bypasses the crash history

Promoting the same files or editing delivery policy does not clear a file's
crash history.

If you need to retry the same file ID:
