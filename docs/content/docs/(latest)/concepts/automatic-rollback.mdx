...
when your runtime considers the app ready, then choose when to check and stage
updates. The optional [`HotUpdater.wrap()`](/docs/react-native-api/wrap) flow
connects the same lifecycle handling to its automatic startup check.
...
This is why Hot Updater must be configured with `HotUpdater.init()` or
`HotUpdater.wrap()` for rollback support. These APIs connect bundle verification
