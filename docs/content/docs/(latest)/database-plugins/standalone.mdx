...
These are the default routes used by `standaloneRepository`. Bundle routes can
be remapped with the `routes` option shown above. The normalized Channel route
is always `/api/channels` so it cannot be derived from or coupled to a custom
bundle list path.
...
| GET    | `/api/channels`     | none                 | `ChannelsResponse`      |
| POST   | `/api/channels`     | `ChannelInsertInput` | `ChannelInsertResponse` |
| DELETE | `/api/channels/:id` | none                 | `ChannelDeleteResponse` |
...
- `DELETE /api/channels/:id` deletes an empty Channel and returns
  ...
  and `409` with reason `not_empty` when a bundle still references the Channel.
...
`/hot-updater/api/bundles*` and the Channel listing route at
`/hot-updater/api/channels` (GET, POST, and DELETE). `features.bundles: true` is
the explicit opt-in for those management APIs; protect them at the framework
or reverse proxy layer, and configure matching `commonHeaders` in
`standaloneRepository`.

`standaloneRepository` remains a bundle-management HTTP client and is used only
in `hot-updater.config.ts`. It does not own Analytics events or client access
keys. The server-side `database` passed to `createHotUpdater` owns those
official domains, and `features` decides whether the self-host runtime exposes
Analytics, enforces client access keys, and mounts the update-check and
bundle-management route groups.

<Callout type="warn">
  The Console cannot read Analytics or manage client access keys through
  `standaloneRepository`; its HTTP contract is deliberately bundle-only. Run
  Console or another protected management surface with direct access to the same
  server-side database domains. Do not pass `standaloneRepository` to
  `createHotUpdater`.
</Callout>
