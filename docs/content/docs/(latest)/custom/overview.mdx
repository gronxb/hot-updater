...
`createHotUpdater` always exposes Analytics ingestion and query capabilities.
React Native clients independently decide whether to send lifecycle events
with `HotUpdater.init({ analytics })`.

Every server must explicitly set `features.clientAccessKeys`. Set it to `true`
to require authentication for update checks and Analytics ingestion through
`database.models.clientAccessKeys`, or `false` to keep those client routes
public.
...
database-commit, and Analytics-query routes live exclusively on
...
Analytics ingestion is available on `handlers.client` and queries are
