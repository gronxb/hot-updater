...
`createHotUpdater` exposes two independent top-level opt-in policies. Both
default to `false`:

- `analytics`: client event ingestion and admin query routes backed by
...
`analytics: true` enables ingestion on `handlers.client` and queries on
`handlers.admin`. The server does not authenticate admin queries itself; the
framework middleware protecting the admin mount owns that decision. `false`
or an omitted `analytics` option disables Analytics routes.
