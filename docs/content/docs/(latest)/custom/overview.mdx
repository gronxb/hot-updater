...
`updateCheck` is enabled by default, and `/version` is always available on
`handlers.client`. Bundle, Release, Release Catalog, Channel, database-commit,
and Analytics-query routes live exclusively on `handlers.admin`. Expose that
handler only through an authenticated framework mount.
...
or an omitted feature disables Analytics routes.
