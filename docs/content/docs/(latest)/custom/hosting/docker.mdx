...
```package-install
npm install @hot-updater/server@rc hono @hono/node-server
```
...
```package-install
npm install -D hot-updater@rc drizzle-kit tsx typescript @types/node
```
...
[Test an OTA update](/docs/guides/simulator-test) against the deployed server;
