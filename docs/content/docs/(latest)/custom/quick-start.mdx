...
// Create this directory before starting the server: mkdir -p data
...
Create `.env.hotupdater` with the management token and R2 credentials used by
both the server and CLI examples:
...
HOT_UPDATER_STORAGE_DOWNLOAD_URL_KEY=replace-with-a-signing-secret
```

## Schema Migration

Create the data directory, generate reviewable Kysely migration SQL, and apply
the current schema before starting the server.
