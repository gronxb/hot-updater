...
Back up Firestore before the schema `1.0.0` migration. It creates Release and
Release-catalog collections, backfills one Release for each legacy Bundle,
compiles every scope, and removes policy from final Bundle documents. Managed
Firebase records its provider migration state separately from the generic SQL
server migrator. The migration is forward-only.

Roll back only to a complete Hot Updater package set documented as compatible
with schema `1.0.0`. Otherwise, stop writers and restore the pre-migration
backup.

### Release catalog cache

Init deploys a Firebase Hosting rewrite for
`/api/check-update{,/**}` in front of the `hot-updater` Function. Use the
Hosting domain so successful catalog responses can be served by Firebase's
...
export default HotUpdater.wrap({
  baseURL: "https://<project-id>.web.app/api/check-update", // [!code ++]
