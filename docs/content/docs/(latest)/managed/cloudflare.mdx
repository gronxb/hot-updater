...
Schema `1.0.0` adds Release and Release-catalog rows, backfills legacy Bundle
policy, and removes desired-state decisions from D1 GET paths. Managed
Cloudflare deployments track applied changes through Wrangler's D1 migration
history; they do not write or consume the generic server migrator's
`schema.core` setting. Keep existing migration files immutable, and back up D1
before applying any pending provider migration.
