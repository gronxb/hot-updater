...
`:channelKey` is the base64url-encoded channel name. Catalog authority is
server-owned and is not part of the React Native configuration or client URL.
...
uses the standard Release Catalog, artifact, Analytics-event, and `/version`
