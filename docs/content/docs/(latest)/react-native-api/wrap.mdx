...
## Advanced: Custom Release Catalog Resolver

For advanced use cases where you need full control over network operations,
such as GraphQL, RPC, or custom authentication, use a custom `resolver` instead
of `baseURL`. A v1 resolver implements both Release Catalog fetch and artifact
resolution; the legacy direct-update `checkUpdate` hook is not accepted.

```tsx
import { HotUpdater, type HotUpdaterResolver } from "@hot-updater/react-native";
import App from "./App";

const resolver: HotUpdaterResolver = {
  authorityId: "custom-api",
  fetchReleaseCatalog: async (params) => {
    const response = await fetch("https://custom-api.com/release-catalog", {
      method: "POST",
      headers: {
  ...
  resolveArtifact: async (params) => {
    const response = await fetch("https://custom-api.com/artifact", {
      method: "POST",
...
> **Note**: The `resolver` option is mutually exclusive with `baseURL`. Both
> resolver methods are required. `resolveArtifact` must return absolute HTTP(S)
> URLs that native code can download directly.
