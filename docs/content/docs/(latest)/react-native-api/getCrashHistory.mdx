...
description: Returns a list of file IDs that previously caused crashes.
icon: AlertTriangle
---
...
Returns an array of file ID strings. Each ID represents a bundle that failed to
load and triggered rollback. These diagnostic values match the console's
Advanced **File ID**, not the update ID from `HotUpdater.getBundleId()`.
...
Track crash frequency and file IDs in your analytics platform.
...
function logCrashHistory() {
  ...
  for (const bundleId of crashes) {
    console.log(`${i}. File ID: ${bundleId}`);
    i++;
  }
