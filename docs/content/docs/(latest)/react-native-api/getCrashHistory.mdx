...
description: Returns a list of artifact IDs that previously caused crashes.
icon: AlertTriangle
---
...
Returns an array of artifact ID strings. Each ID represents a bundle that failed to
load and triggered rollback. These diagnostic values match the console's
Advanced **Artifact ID**, not the update ID from `HotUpdater.getBundleId()`.
...
Track crash frequency and artifact IDs in your analytics platform.
...
function logCrashHistory() {
  ...
  for (const bundleId of crashes) {
    console.log(`${i}. Artifact ID: ${bundleId}`);
    i++;
  }
