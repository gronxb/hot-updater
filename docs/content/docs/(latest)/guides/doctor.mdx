...
- Signing settings match `HOT_UPDATER_PUBLIC_KEY` in `Info.plist` and
  `com.hotupdater.PUBLIC_KEY` in Android application metadata
...
`HOT_UPDATER_CHANNEL` in `Info.plist` or `com.hotupdater.CHANNEL` in Android
application metadata exists, the command reports it as context only. Android
`strings.xml` values such as `hot_updater_channel` are still read for legacy
projects, but new setup uses `AndroidManifest.xml`.
