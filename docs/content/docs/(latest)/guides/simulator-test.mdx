...
Open the Bundle in the Console and set its `enabled` policy to `false`. An
affected installation moves to a previous compatible enabled Bundle or to its
built-in Bundle when no compatible Bundle remains.
...
continues to track failed artifact IDs on the device.
