...
## Step 3: Disable a Release

<video src={"/docs/console/console.mp4"} controls autoPlay loop muted />

Open the Release in the Console and set its `enabled` policy to `false`. An
affected installation moves to a previous compatible enabled Release or to its
built-in Bundle when no compatible Release remains.

```package-install
npx hot-updater console
```

The same operational action is available from the CLI when you already know
the ID shown in the console or deployment output:

```package-install
npx hot-updater release disable <id>
```

This policy rollback is different from automatic startup-crash recovery, which
continues to track failed file IDs on the device.
