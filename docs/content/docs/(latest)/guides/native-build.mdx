...
Hot Updater provides CLI commands to create store-ready artifacts or launch the app on a device. `build:android`, `build:ios`, `run:android`, and `run:ios` reuse the same configuration defined in `hot-updater.config.ts` and respect your update strategy. The `build:*` commands copy artifacts into an output directory, while the `run:*` commands build, install, and launch directly.
