...
Hot Updater ships dedicated native helpers so you can create store-ready
artifacts or boot the app on a device without leaving the CLI. All four
commands reuse the schemes in `hot-updater.config.ts` and respect your update
strategy. The `build:*` commands also copy artifacts to a predictable output
directory for further automation; the `run:*` commands build, install, and
launch directly.

`build:android` is available normally. `build:ios`, `run:android`, and
`run:ios` are experimental and are registered only when `EXPERIMENTAL=1` is
set.
...
- Run `hot-updater channel set <channel>` so `HOT_UPDATER_CHANNEL` exists in
  `Info.plist` and `com.hotupdater.CHANNEL` exists in the Android application
  metadata.
...
All commands support `-s/--scheme` and `-i/--interactive`. The scheme is
required unless interactive mode is used. `-o/--output-path` is meaningful for
`build:*` commands, which copy their result to the selected directory.
...
```
Options:
  -d, --device [deviceName | deviceId]  select an Android device or emulator
...
```
Options:
  -d, --device [deviceName | deviceId]  select an Apple device or simulator by name or UDID
```
...
For `build:android` and `build:ios`, artifacts default to
`.hot-updater/output/build/<platform>/<scheme>`. The directory is recreated on
every build, so point `-o` to a different path if you need to keep older
artifacts. Copying happens after build plugin hooks complete, giving CI
pipelines a stable handoff point. The `run:*` commands do not copy persistent
artifacts or invoke build-plugin native hooks.
