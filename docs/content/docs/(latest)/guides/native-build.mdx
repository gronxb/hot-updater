...
- Confirm the [default channel](/docs/guides/channel-management) before building.
  Run `hot-updater channel set <channel>` when choosing a channel other than the
  default `production`.
...
```package-install
EXPERIMENTAL=1 npx hot-updater run:android -s releaseApk -d "Pixel_6_Pro_API_35" --port 8082
```
