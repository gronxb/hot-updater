...
An AI agent can install the required packages as part of the next step. For
terminal setup, install the hot-updater package:
...
Choose how to set up your provider:

<Tabs items={["AI agent", "Terminal"]}>
  <Tab value="AI agent">

Install the [Hot Updater skill](/docs/guides/ai-agents) in your coding agent:

```sh
npx skills add hot-updater/skills
```

Then ask it to set up infrastructure for your project:
...
configuration and asks which provider to use if it cannot determine one. It asks
for any other missing choices or access, generates templates with
`hot-updater agent infra setup`, and applies them using available provider tools.
It verifies each step and resumes unfinished work after checking remote state.
It creates missing projects and resources itself when the available tools and
access permit it. If login is required, authenticate with the provider or a local
credential file; do not paste tokens or passwords into the conversation.
See [Agent Infrastructure](/docs/guides/agent-infrastructure) for the workflow.
...
After setup, review the generated or merged local configuration:
...
<Callout type="info">
  Re-running `init` with a different provider updates only the provider-managed
  imports, helpers, and `build`/`storage`/`database` settings in
  `hot-updater.config.ts`. It preserves your other configuration, previous
  provider variables in `.env.hotupdater`, and existing remote infrastructure.
  Remove old credentials and infrastructure manually only after all deployed
  app versions have migrated to the new provider.
</Callout>
...
export default HotUpdater.wrap({
  ...
  requestHeaders: {
    "x-api-key": "<HOT_UPDATER_API_KEY from .env.hotupdater>", // Required for managed providers
  },
...
<Callout type="info">
  Setup provides the update server URL. Check the agent's verified deployment
  record, terminal output, or provider console for the correct URL. Managed
...
React Native clients identify the client handler only through `baseURL`.
