...
`standaloneRepository` owns only the remote database contract. The separate
`standaloneAnalytics` Console runtime plugin lets the local Console proxy
Analytics through the same remote configuration without decorating the
database plugin. It configures only the local Console runtime and is not
forwarded to the remote deployment handler or its `createHotUpdater` plugins.
