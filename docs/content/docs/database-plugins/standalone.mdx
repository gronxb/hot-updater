...
| `enabled`                 | `"true" \| "false"`  | Exact enabled-state match                                      |
...
| `idIn`                    | repeated `string`    | Include each value as a repeated query parameter               |
| `targetAppVersion`        | `string \| "null"`   | Exact match; the literal `null` matches a database null        |
| `targetAppVersionIn`      | repeated `string`    | Match any supplied app version; repeat the query parameter     |
| `targetAppVersionNotNull` | `"true" \| "false"`  | When true, require a non-null target app version               |
| `fingerprintHash`         | `string \| "null"`   | Exact match; the literal `null` matches a database null        |

Array filters are encoded as repeated parameters, for example
`?idIn=bundle-1&idIn=bundle-2`. Boolean values must be the literal
`true` or `false`; other spellings return a bad request. The
literal `null` is reserved for the nullable string filters above.
