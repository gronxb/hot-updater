...
See the [Self-Hosting (Custom) Guide](/docs/custom/overview) for complete server implementation instructions using `@hot-updater/server` with database middle layers (Drizzle, Prisma, Kysely), native MongoDB or S3 options, and frameworks (Hono, Express, Elysia).
</Callout>
...
| `after` | `string` | Fetch the next window after this opaque cursor token |
| `before` | `string` | Fetch the previous window before this opaque cursor token |
...
- [Database Middle Layers](/docs/custom/database/drizzle) - Choose Drizzle, Prisma, or Kysely; use native MongoDB or S3 options when you do not need a middle layer
