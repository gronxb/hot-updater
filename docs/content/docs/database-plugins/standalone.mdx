...
`analytics()` is the only Analytics plugin. `standaloneRepository` reuses its
remote URL, headers, and route overrides through an internal transport
capability when Analytics is installed, so there is no second Standalone
Analytics plugin or duplicated configuration. Constructing the repository
alone does not install Analytics or make a network request. `console.plugins`
configures only the local Console runtime and is not forwarded to the remote
deployment handler.
...
Standalone validates the upstream Analytics route flags through `GET /version`.
It caches a validated response for 30 seconds and coalesces concurrent
refreshes. Capability requests time out after five seconds. If a refresh fails,
the last validated capability is used for at most five minutes; without a
recent validated response, Analytics routes fail closed.
...
<Callout type="info">
  ...
  routes are explicitly enabled, including `GET /api/bundles/channels`.
</Callout>
...
`/hot-updater/api/bundles*`. `routes.bundles` is the explicit opt-in for
those management APIs. The public access policy shown delegates protection to
the framework or reverse proxy; configure matching `commonHeaders` in
`standaloneRepository`.
The installed `analytics()` plugin independently mounts
`POST /hot-updater/events` and the Analytics routes consumed by Console under
`/hot-updater/api/bundles/*/events*` and `/hot-updater/api/installations*`.
Standalone reads the upstream `/version` response before forwarding Analytics
requests, so upstream `eventIngestion` and `analyticsQueries` availability is
