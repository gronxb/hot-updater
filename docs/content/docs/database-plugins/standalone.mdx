...
`routes.analytics: true` independently mounts `POST /hot-updater/events` and
the Analytics routes consumed by Console under
`/hot-updater/api/bundles/*/events*` and `/hot-updater/api/installations*`.
Enabling it with an unsupported database logs a warning and mounts none of
those routes.
