...
`GET /api/bundles` must return a paginated JSON body:
...
`standaloneRepository` reads pagination metadata from the response body, and
`getChannels()` uses `GET /api/bundles/channels`.
