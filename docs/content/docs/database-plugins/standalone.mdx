...
interface Routes {
  /**
   * @deprecated Use `create` and `update`.
   */
  upsert?: () => RouteConfig;
  create?: () => RouteConfig;
...
- If you only provide `routes.upsert`, update requests are sent as `POST` to
  that route with a `Bundle[]` body.
...
`GET /api/bundles` must accept pagination and filter query params and return a
paginated JSON body.
...
| `idEq` | `string` | Exact ID match |
| `idGt` | `string` | ID greater than |
| `idGte` | `string` | ID greater than or equal |
| `idLt` | `string` | ID less than |
| `idLte` | `string` | ID less than or equal |
| `idIn` | `string[]` | Repeat the query param for multiple IDs |
| `targetAppVersion` | `string` | Exact match |
| `targetAppVersionNull` | `"true"` | Filter rows where `targetAppVersion` is `null` |
| `targetAppVersionIn` | `string[]` | Repeat the query param for multiple values |
| `targetAppVersionNotNull` | `"true"` | Filter rows where `targetAppVersion` is not `null` |
| `fingerprintHash` | `string` | Exact match |
| `fingerprintHashNull` | `"true"` | Filter rows where `fingerprintHash` is `null` |
...
does not derive pagination from headers.
...
For backward compatibility, `standaloneRepository` also accepts the legacy flat
response shape `{ "channels": string[] }`.
