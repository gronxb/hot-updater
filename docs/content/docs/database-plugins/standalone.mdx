...
`routes.analytics: true` independently mounts the Analytics routes consumed by
Console under `/hot-updater/api/bundles/*/events*` and
`/hot-updater/api/installations*`. When the database supports Analytics,
`POST /hot-updater/events` is mounted automatically for client telemetry.
