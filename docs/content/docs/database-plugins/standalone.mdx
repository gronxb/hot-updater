...
`standaloneAnalytics` manifest lets the local Console proxy Analytics through
the same remote configuration without decorating the database plugin.
...
`/hot-updater/api/bundles*`. `coreRoutes.bundles` is the explicit opt-in for
