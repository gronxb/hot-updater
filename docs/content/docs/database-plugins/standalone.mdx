...
`/hot-updater/api/bundles*`. `features.bundles: true` is the explicit opt-in for
...
official domains, and `features` decides which route groups and database-backed
capabilities the self-host runtime exposes.
