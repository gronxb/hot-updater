...
Cursor-based pagination is the official contract. Legacy offset-based
pagination remains supported for backwards compatibility, but it is deprecated
...
| `offset` | `number` | Deprecated legacy page offset |

If both cursor params and `offset` are provided, cursor pagination should take
precedence.
