...
| `idEq`, `idGt`, `idGte`, `idLt`, `idLte` | `string` | Bundle ID filters |
| `idIn` | `string[]` | Repeated parameter |
| `targetAppVersion` | `string \| null` | Exact match, or literal `null` |
| `targetAppVersionIn` | `string[]` | Repeated parameter |
| `targetAppVersionNotNull` | `boolean` | Exact match |
| `fingerprintHash` | `string \| null` | Exact match, or literal `null` |
...
Array filters use repeated query parameters. Boolean values must be exactly
`true` or `false`, and nullable string filters use the literal `null`.
