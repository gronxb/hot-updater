...
`createHotUpdater` exposes three configurable route groups:
...
- `analytics`: Analytics and installation query endpoints used by Console

`updateCheck` is mounted by default. `bundles` and `analytics` are disabled by
default and must be enabled explicitly. Protect all enabled `/api/*` management
routes with framework middleware. The `/version` endpoint is always mounted
for diagnostics. Event ingestion is controlled separately by
`eventIngestion.authorize` so a public update server can accept reports without
