...
Install the server and standalone packages.
...
Self-hosted mode lets you control the database layer while using existing cloud storage adapters.
...
- **Storage**: Uses existing adapters (AWS S3, Supabase, Cloudflare R2, Firebase)
...
The `storages` field in your server configuration accepts storage adapters that
implement the `runtime` profile. Runtime storage adapters translate stored bundle
...
download operations. An adapter such as `s3Storage` can support both profiles,
while edge providers such as Cloudflare Workers may use a separate runtime
adapter that reads from platform bindings.

### Multiple Storage Adapters

`storages` is an array, allowing you to configure multiple storage providers simultaneously. Each adapter acts as a decoder for its specific storage URI format.
...
The server automatically uses the correct adapter to decode each `storageUri` based on its format.
...
- `bundles`: bundle management endpoints used by the CLI `standaloneRepository` adapter
...
For most use cases, use only the storage adapter that matches your CLI's `hot-updater.config.ts`.
...
This ensures consistency and simplifies configuration. Only use multiple adapters when you need to support bundles from different storage providers.
</Callout>

**Available official adapters:**
- [s3Storage](../storage-adapters/aws) - AWS S3 and Cloudflare R2
- [supabaseStorage](../storage-adapters/supabase) - Supabase Storage
- [firebaseStorage](../storage-adapters/firebase) - Firebase Cloud Storage
