...
- `bundles`: write-capable database and bundle management endpoints

`updateCheck` is mounted by default. `bundles` is disabled by default and must
be enabled explicitly when the CLI needs the `standaloneRepository` database
API. When `bundles: true`, protect `/api/database/v2/*` and `/api/bundles*`
with framework middleware. The
