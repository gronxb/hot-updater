...
`createHotUpdater` exposes three route groups:
...
`updateCheck` and `version` are mounted by default. `bundles` is disabled by
default and requires an authorization hook when enabled, unless you protect
`/hot-updater/api/*` with framework middleware before mounting the handler.
