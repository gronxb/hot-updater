...
default and must be enabled explicitly. Protect all routes with framework
middleware. The `/version` endpoint is always mounted for diagnostics. When the
database supports Analytics, event ingestion is mounted automatically and
remains independent from Analytics query routes.
