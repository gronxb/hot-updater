---
title: "Overview"
description: Build a custom backend server for Hot Updater database with any framework and middle layer.
icon: Server
version: "v0.22.0"
---
...
type RuntimeContext = {
  readonly env?: {
    readonly HOT_UPDATER_EVENT_TOKEN?: string;
  };
};

export const hotUpdater = createHotUpdater<RuntimeContext>({
  database: kyselyDatabase({ db, provider: "sqlite" }),
  storages: [
    s3Storage({
      /* AWS S3 config */
    }),
...
The `/bundle-events/app-ready` endpoint is not mounted by default. Enable it
with `bundleEvents` only when the selected database implements bundle events,
and enforce authentication, authorization, rate limits, or quotas in the
deployment-owned `policy` hook.
...
export const hotUpdater = createHotUpdater({
  ...
  bundleEvents: {
    ...
    policy: (request, context) => {
      ...
      const token =
        context.env?.HOT_UPDATER_EVENT_TOKEN ??
        process.env.HOT_UPDATER_EVENT_TOKEN;
    ...
    retention: { maxAgeMs: 30 * 24 * 60 * 60 * 1000 },
  },
});
```

Use a durable deployment service for shared rate and quota state; an in-memory
counter does not coordinate across server instances. The policy receives the
current runtime context, so Workers can use KV, Durable Objects, rate-limit
bindings, and secrets from `context.env`. Kysely, Drizzle, and Prisma adapters
support cutoff-based event retention. The React Native client treats `404` and
`501` from this optional endpoint as a no-op, while other failures remain
visible.
...
Choose one of the official middle layers first, then select the database provider or dialect underneath it:

- [Drizzle](./database/drizzle) - TypeScript ORM middle layer with SQL-like syntax
- [Prisma](./database/prisma) - ORM middle layer with intuitive data modeling
- [Kysely](./database/kysely) - Type-safe query-builder middle layer

Native database options are available when you do not need an ORM or
query-builder layer:
...
- **Official middle layers**: Drizzle, Prisma, and Kysely with provider- or dialect-specific database setup
- **Native database options**: MongoDB driver or S3-compatible metadata storage
- **Database control**: PostgreSQL, MySQL, SQLite, MongoDB, or S3-compatible storage on your infrastructure
