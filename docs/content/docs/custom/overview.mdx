...
```package-install
npm install @hot-updater/analytics @hot-updater/server @hot-updater/standalone --save-dev
```
...
The `console.plugins` array installs features only in the local Console
runtime. It does not propagate them into this self-hosted server, a managed
preset, or any separately constructed `createHotUpdater` handler.
...
`createHotUpdater` exposes two configurable core route groups:
...
`updateCheck` is mounted by default and `bundles` is disabled by default.
Analytics is a separate feature manifest that consumes the generic database
runtime. Protect
management and query routes with a kernel authentication plugin, or declare
them public explicitly and protect them with framework middleware. The
...
If you want only update checks and `/version`, omit Analytics and disable the
