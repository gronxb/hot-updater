...
The `storages` field in your server configuration accepts storage plugins that implement the `runtime` profile. Runtime storage plugins act as decoders/interpreters for storage URIs and as direct readers for small manifest metadata.
...
3. When manifest metadata is available, the server reads `manifest.json` through `runtime.readText` to calculate `manifestUrl`, `changedAssets`, and patch metadata
4. React Native clients download the bundle and changed assets using the returned URLs
...
`createHotUpdater` mounts three route groups by default:

- `updateCheck`: update-check endpoints used by React Native clients
- `version`: lightweight `/version` endpoint for diagnostics and health checks
...
For managed or public-facing runtimes that should serve only update checks, also
set `version: false`.
