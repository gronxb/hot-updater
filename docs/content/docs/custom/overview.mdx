...
The `storages` field accepts flat storage objects whose `get` operation returns
a Web `Response`. `storageDelivery` separately chooses between the built-in
streaming route and a provider/CDN URL resolver.
...
2. When the API returns bundle information, `storageDelivery` turns the stored URI into an HTTP(S) URL, either through the built-in streaming route or a provider CDN resolver
...
- `bundles`: bundle management endpoints used by the CLI `standaloneRepository`

Optional server behavior is configured separately under `features`:

- `analytics`: mounts event ingestion and Analytics query routes backed by
  `database.analytics`
- `clientAccessKeys`: authenticates update checks and Analytics ingestion
  through `database.clientAccessKeys`

This separation is intentional: `routes` decides which core API groups exist,
while `features` enables cross-cutting behavior backed by official database
domains.
...
`/version` endpoint is always mounted for diagnostics.
