...
`createHotUpdater` mounts two route groups by default:

- `updateCheck`: update-check endpoints used by React Native clients
- `bundles`: bundle management endpoints used by the CLI `standaloneRepository` plugin, plus `/version`
...
If you want to expose only update-check endpoints, disable the `bundles` group:
...
This is useful for managed or public-facing runtimes that should only serve update checks.
