...
- `analytics`: client event ingestion and Analytics/installation queries
...
middleware. The `/version` endpoint is always mounted for diagnostics. Enabling
`analytics` with an unsupported database logs a warning and mounts no Analytics
routes.
