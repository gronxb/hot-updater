...
can resolve stored bundle locations into client download URLs. They can also
read small metadata files directly from storage when bundle diffing data is
available.
...
2. When the API returns bundle information, the server decodes the `storageUri` using `getDownloadUrl` to generate an HTTP(S) download URL
...
The CLI `storage` field needs upload/delete/local file download operations. A
plugin such as `s3Storage` can support every storage operation, while edge
providers such as Cloudflare Workers may use a separate storage plugin that
reads from platform bindings.
