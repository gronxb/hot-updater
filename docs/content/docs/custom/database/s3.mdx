...
<Callout type="warn">
`s3Database` does not expose event ingestion or analytics endpoints because it
cannot safely append concurrent events. Use a supported row- or document-based
database adapter if you need [Analytics](/docs/guides/analytics).
</Callout>
