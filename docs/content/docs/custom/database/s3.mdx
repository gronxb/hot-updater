...
<Callout type="warn">
  `s3Database` does not support OTA analytics. It replaces a shared
  metadata snapshot for each mutation, so concurrent event reports can cause
  write conflicts. Use a row-oriented database adapter for analytics. The
  Console hides Analytics and installation surfaces because the capability is
  absent, not because it recognizes the S3 adapter name.
</Callout>
...
Bundle-event ingestion, analytics, and installation-history endpoints are not
mounted because this snapshot adapter does not expose the optional
Analytics capability. Independent append writes are unsafe for its snapshot
storage model.
