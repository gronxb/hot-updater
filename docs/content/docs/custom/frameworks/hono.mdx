...
If you set `coreRoutes: { updateCheck: true, bundles: false }` and omit
Analytics from `plugins`,
`/hot-updater/api/*` is not mounted and `/hot-updater/version` stays
available for diagnostics.
Enable public `coreRoutes.bundles` only when this server accepts
bundle-management requests; then the `/hot-updater/api/*` middleware above
protects those routes.
