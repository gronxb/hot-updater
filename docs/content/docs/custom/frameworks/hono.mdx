...
If you set
`routes: { updateCheck: true, bundles: false, analytics: false }`,
...
Analytics query endpoints are mounted when `routes.analytics` is enabled and
the database supports Analytics:
