...
new Elysia()
  ...
  .onBeforeHandle(({ bearer, request }) => { // [!code ++]
    ...
    if (pathname === "/hot-updater/api" || pathname.startsWith("/hot-updater/api/")) { // [!code ++]
      if (bearer !== process.env.HOT_UPDATER_AUTH_TOKEN) { // [!code ++]
...
Event ingestion and Analytics query endpoints are mounted when
`routes.analytics` is enabled and the database supports Analytics:
