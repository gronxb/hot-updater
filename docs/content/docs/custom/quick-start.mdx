...
By default, `createHotUpdater` mounts both update-check routes and bundle management routes used by the CLI `standaloneRepository` plugin. If you want this server to expose only update checks, add:
...
<Callout type="warn">
With Hono `mount()`, keep `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public for React Native clients. If `routes.bundles` is enabled, protect `/hot-updater/version` and `/hot-updater/api/*` with authentication.
</Callout>
