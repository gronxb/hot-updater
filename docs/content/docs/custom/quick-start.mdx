...
enabled explicitly and protected. The example above enables them and authorizes
management requests with `authorizeBundleRequest`.
...
They are disabled unless `routes.bundles` is enabled, and
`authorizeBundleRequest` must allow each management request. The server example
above uses Bearer authentication through the Hot Updater authorization hook. If
you prefer framework middleware, protect `/hot-updater/api/*` before mounting
the handler.
