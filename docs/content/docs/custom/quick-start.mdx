...
<Callout type="warn">
With Hono `mount()`, keep `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public for React Native clients. `/hot-updater/version` stays mounted by default for diagnostics, so leave it public or disable it with `routes.version: false`. Protect `/hot-updater/api/*` with authentication whenever `routes.bundles` is enabled.
</Callout>
