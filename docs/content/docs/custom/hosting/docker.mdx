...
export const hotUpdater = createHotUpdater({
  database: drizzleDatabase({ db, provider: "postgresql", schema }),
  storages: [
...
Generate the Drizzle schema for Hot Updater tables into `src`.
...
Generate a migration for review:

```bash
npx drizzle-kit generate
```

Alternatively, push the schema directly to a development database:
...
Push the generated schema from the development checkout, where `drizzle-kit`
is installed. Use the host-mapped PostgreSQL port for this command.
...
Build and start the Hot Updater service after the schema is ready.
