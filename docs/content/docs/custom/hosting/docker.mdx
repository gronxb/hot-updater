...
<Callout type="warn">
When using Hono `mount()`, keep `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public. `/hot-updater/version` is always mounted for diagnostics, so leave it public. Protect `/hot-updater/api/*` when `routes.bundles: true`.
</Callout>
