...
<Callout type="warn">
When using Hono `mount()`, keep `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public. If you expose bundle routes, protect `/hot-updater/version` and `/hot-updater/api/*`.
</Callout>
