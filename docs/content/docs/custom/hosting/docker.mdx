...
<Callout type="warn">
When using Hono `mount()`, keep `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public. `/hot-updater/version` is mounted by default for diagnostics, so leave it public or disable it with `routes.version: false`. Protect `/hot-updater/api/*` whenever bundle routes are enabled.
</Callout>
