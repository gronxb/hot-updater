...
<Callout type="warn">
With Hono `mount()`, leave `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public for update checks. `/hot-updater/version` is always mounted for diagnostics, so leave it public. Protect `/hot-updater/api/*` when bundle routes are enabled.
</Callout>
