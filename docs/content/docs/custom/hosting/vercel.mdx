...
<Callout type="warn">
With Hono `mount()`, leave `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public for update checks. `/hot-updater/version` is mounted by default for diagnostics, so leave it public or disable it with `routes.version: false`. Protect `/hot-updater/api/*` when bundle routes are enabled.
</Callout>
