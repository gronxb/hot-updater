...
<Callout type="warn">
With Hono `mount()`, leave `/hot-updater/app-version/*` and `/hot-updater/fingerprint/*` public for update checks. Protect `/hot-updater/version` and `/hot-updater/api/*` if bundle routes are enabled.
</Callout>
