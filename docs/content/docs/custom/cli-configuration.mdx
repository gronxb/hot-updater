...
<Callout type="warn">
**Critical: Storage Adapter Must Match**

The `storage` adapter in your CLI config must match the `storages` in your server's `createHotUpdater` configuration.

**CLI uses `s3Storage` → Server must use `s3Storage`**

Both sides need the same adapter type and configuration to encode/decode storage URIs correctly.
</Callout>

## Standalone Repository Configuration

The `standaloneRepository` adapter connects your CLI to your self-hosted server:
...
**Choose your build and storage adapters:**
- [Build Adapters](../build-adapters/bare) - React Native CLI, Expo, Re.Pack
- [Storage Adapters](../storage-adapters/supabase) - AWS S3, Supabase, Firebase, Cloudflare R2
