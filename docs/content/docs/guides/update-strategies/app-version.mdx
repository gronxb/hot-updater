...
```
GET {baseUrl}/v2/release-catalogs/app-version/:authorityId/:platform/:channelKey/:appVersion
```

It evaluates the catalog locally, then resolves the selected artifact through
`/v2/artifacts/:targetBundleId/from/:currentBundleId`. `:channelKey` is the
...
```bash
curl "https://your-update-endpoint.com/check-update/v2/release-catalogs/app-version/default/ios/cHJvZHVjdGlvbg/1.0.0"
```
