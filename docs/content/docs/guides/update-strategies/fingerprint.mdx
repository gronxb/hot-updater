...
`appVersion` is the default update strategy. Fingerprint targeting is opt-in,
...
- Android: application-level `AndroidManifest.xml` metadata named
...
Most projects do not need custom paths because Hot Updater discovers native
files automatically. `stringResourcePaths` remains a deprecated read fallback
for older projects.
