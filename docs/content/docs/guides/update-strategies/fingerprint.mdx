...
`appVersion` is the default update strategy. Fingerprint targeting is opt-in, so configure it explicitly:
...
- Android: application-level `AndroidManifest.xml` metadata named `com.hotupdater.FINGERPRINT_HASH`
...
`stringResourcePaths` remains a deprecated read fallback for older projects.
