...
base bundles. Set `patch.enabled: false` to disable it, or change
`patch.maxBaseBundles` from its default of `3`. Patch generation is best-effort;
a skipped patch does not fail the deployment because the normal archive update
remains available.
