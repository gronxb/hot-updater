...
- Android: application-level `AndroidManifest.xml` metadata → `com.hotupdater.PUBLIC_KEY`
...
The CLI can still read `hot_updater_public_key` from configured `stringResourcePaths` as a deprecated fallback for older projects.
