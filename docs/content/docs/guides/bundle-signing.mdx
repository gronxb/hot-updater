...
> **Note**: Use EAS Secrets instead of `eas.json` for better security. The private key in `eas.json` may be visible in version control.
...
4. Skip with warning → Manual configuration needed
