...
- Android: application-level `AndroidManifest.xml` metadata →
...
The CLI can still read `hot_updater_public_key` from configured
`stringResourcePaths` as a deprecated fallback. Export and removal commands
prefer manifest metadata and clean up the legacy string when a manifest is
available.
