...
The `examples/v0.81.0` sample app exposes this value in the UI as:

- `Runtime Details > Cohort`
- `Runtime Details > Cohort Summary`
- `Actions > Current Cohort Summary`
...
<Callout type="warn">
  ...
  first and store it before overriding. The sample app does this so
  `Restore Initial Cohort` can revert the change.
</Callout>
