...
On Android, `doctor` reads application-level `AndroidManifest.xml` metadata:
`com.hotupdater.CHANNEL`, `com.hotupdater.FINGERPRINT_HASH`, and
`com.hotupdater.PUBLIC_KEY`. Configure non-standard locations with
`platform.android.androidManifestPaths`. Legacy `strings.xml` entries are
read only as a deprecated fallback through `stringResourcePaths`.
