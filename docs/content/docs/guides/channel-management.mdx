  ...
  platform: {
    ...
    android: {
      androidManifestPaths: [
        ...
        "android/app/src/internal/AndroidManifest.xml", // Include another variant
      ],
    },
  },
});
...
`stringResourcePaths` is a deprecated read fallback for projects that still
store Hot Updater values in `strings.xml`. New writes use application-level
`AndroidManifest.xml` metadata and remove the corresponding legacy string.
