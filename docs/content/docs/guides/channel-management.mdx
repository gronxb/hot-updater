  ...
  platform: {
    ...
    android: {
      androidManifestPaths: [
        ...
        "android/app/src/debug/AndroidManifest.xml", // Include additional variant files
      ],
    },
  },
});
...
`stringResourcePaths` is a deprecated read fallback for projects that still store Hot Updater values in `strings.xml`. New writes use application-level `AndroidManifest.xml` metadata.
