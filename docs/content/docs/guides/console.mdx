...
| ----------------- | -------------------------------------------------------- | ------- |
| `console.plugins` | Installs feature plugins in the local Console runtime    | `[]`    |
...
export default defineConfig({
  console: { // [!code hl]
    plugins: [analytics({ queryAccess: "public" })], // [!code hl]
    port: 3000, // [!code hl]
  }, // [!code hl]
  build: ...,
  storage: ...,
...
`analytics()` is the only Analytics plugin. Generic databases use their
concurrency-safe `bundle_events` contract, while `standaloneRepository`
automatically supplies its remote Analytics transport when the same
`analytics({ queryAccess: "public" })` manifest is installed. See the
[Standalone Database guide](/docs/database-plugins/standalone).

`console.plugins` belongs only to the local Console runtime. These manifests
are passed to the Console's internal `createHotUpdater` instance; they are not
forwarded to a separately deployed handler, a managed provider preset, or any
other `createHotUpdater` call.

This array is also not an access-control mechanism for the Console UI or its
RPC endpoints. Adding `betterAuthPlugin(...)` to `console.plugins` does not
authenticate access to the Console itself; secure Console access at the
network or hosting boundary.
