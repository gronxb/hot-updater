...
Use `analytics({ queryAccess: "public" })` only when the configured database
implements the complete, concurrency-safe `bundle_events` contract.
Standalone users must install `standaloneAnalytics(...)` instead; see the
