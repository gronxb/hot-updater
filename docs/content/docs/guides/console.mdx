...
| ------------------- | -------------------------------------------------------- | ------------ |
| `console.analytics` | Installs Console Analytics from the database or a feature manifest | `"disabled"` |
...
Use `"database"` only when the configured database implements the complete,
concurrency-safe `bundle_events` contract. Standalone users must pass
`standaloneAnalytics(...)` instead; see the
