...
description: "Opens a localhost server based on adapters configured in `hot-updater.config.ts`. For security reasons, only localhost server is supported at the moment."
icon: Terminal
---
...
The `storage` and `database` adapters must be exist.
