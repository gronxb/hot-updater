...
$hot-updater inspect a Release, then disable that exact Release
$hot-updater show which Releases reference this Bundle
...
- For state-changing operations such as deploy, Release enable/disable, or
  database migration, the agent should identify the Release ID and affected
  scope first and verify the result afterward. Operational rollback uses
  `release disable <release-id>`; Bundle IDs remain artifact and native crash
  identities.
