...
- `download()` downloads private R2 objects to the provided local file path
  through Wrangler. This is intended for deploy-time tooling such as partial
  update generation.
- `getDownloadUrl()` is not supported because R2's Wrangler API cannot provide
  an HTTP(S) presigned URL for runtime/browser downloads.
- For public presigned URLs, use `s3Storage` from `@hot-updater/aws` with R2 S3
  API credentials.
