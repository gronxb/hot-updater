...
`s3Storage` implements byte upload, streaming Web-response reads, existence
checks, and exact deletion. HTTP URL generation is deliberately separate. Use
`cloudFrontStorageDelivery` for the managed Lambda@Edge/CDN path, or the
built-in `createHotUpdater` delivery route in a self-hosted server.
