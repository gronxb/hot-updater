...
description: Store your Hot Updater bundles in AWS S3 or another S3-compatible storage service.
icon: aws
---
...
For a new Cloudflare R2 setup, use the native
[`r2Storage`](/docs/storage-plugins/cloudflare) plugin. It uses R2's S3 API
credentials while keeping the provider-specific setup explicit.

Use `s3Storage` with R2 only when you intentionally need `s3://` storage URIs
or its object-listing and batch-deletion support for `hot-updater storage
prune`.
...
- `node`: uploads, checks, deletes, and downloads local files during CLI/console workflows.
