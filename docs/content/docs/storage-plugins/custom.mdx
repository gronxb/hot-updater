...
Once you've created your custom storage plugin, use it in your `createHotUpdater` instance. This is the server-side configuration that handles update requests from your React Native app:
...
For a complete server setup example, see the [Quick Start](/docs/custom/quick-start) guide.

### CLI Configuration

To deploy bundles using your custom storage plugin, configure your CLI in `hot-updater.config.ts`:
...
The `source` parameter should point to your update server's endpoint (configured with `createHotUpdater`). The update server uses your custom storage plugin to serve bundles:

1. Checks if updates are available for the client's current version/channel
2. Uses your custom storage plugin's `getDownloadUrl()` method to generate download URLs
3. Serves bundle metadata to the client

For more information on setting up an update server, see the [Quick Start](/docs/custom/quick-start) guide.
