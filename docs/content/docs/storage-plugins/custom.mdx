...
```typescript
async getDownloadUrl(storageUri) {
  // Parse and validate the storage URI
  const url = new URL(storageUri);
  const protocol = url.protocol.replace(":", "");

  if (protocol !== "my-storage") {
    throw new Error("Invalid storage URI protocol");
  }
  ...
  if (!bucket || !key) {
    throw new Error("Invalid storage URI: missing bucket or key");
  }
