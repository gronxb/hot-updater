...
`supabaseStorage` implements byte upload, Web-response reads, existence checks,
and exact deletion. Signed URLs are provided by the separate
`supabaseStorageDelivery`. For Supabase Edge Functions, import
the same `supabaseStorage` name from `@hot-updater/supabase/edge`; that
implementation captures the edge-compatible client configuration without
