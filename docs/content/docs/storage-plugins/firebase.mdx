...
`firebaseStorage` implements byte upload, Web-response reads, existence checks,
and exact deletion. `firebaseStorageDelivery` owns CDN or signed URL generation;
the core storage contract has no Functions, URL, or request-context dependency.
