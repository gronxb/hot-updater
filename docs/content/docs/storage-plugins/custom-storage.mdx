...
| --------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `node`    | `hot-updater deploy`, `hot-updater bundle promote --action copy`, bundle diff generation, and console server actions  | Upload, existence check, local download, and delete           |
  ...
  factory: (config) => ({
    node: {
      ...
      async exists(storageUri) {
        // Replace this with the provider's HEAD/stat operation.
        throw new Error("Implement the backend existence check");
      },
...
Checks whether an object can be reused safely during deploy and copy workflows.

```typescript
exists: (storageUri: string) => Promise<boolean>;
```

Return `false` only when the object is missing. Throw for permission errors,
invalid storage URIs, provider outages, and other failures.
...
Install the SDK packages used by this example as direct dependencies:
      ...
      runtime: {
        ...
        async readText(storageUri) {
          ...
          try {
            ...
            if (!response.Body) {
              throw new Error(
                "S3 returned a successful response without a body",
              );
            }
...
Assign this value to the required `storage` field alongside your config's
...
- `exists` returns `true` for an existing object, `false` for a missing object,
  and rethrows permission or provider errors.
