...
Most providers should use `createUniversalStoragePlugin`. For example, the root
Cloudflare `r2Storage` export is universal with R2 S3-compatible credentials.
The package also exposes a runtime-only Worker binding implementation from
`@hot-updater/cloudflare/worker` when credentials are not appropriate.
