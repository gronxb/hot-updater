...
as long as the implementation only uses APIs available in that runtime.
Cloudflare Worker-specific storage can read runtime bindings with
`cloudflare:workers` instead of passing request context into storage methods.
...
For Workers, Pages, or other edge runtimes, keep the storage config limited to
serializable options and let the runtime-specific module read platform
bindings:
