...
No additional Hot Updater Babel plugin is required for standard Expo apps.
...
Expo DOM components are designed for EAS Update. If you use `use dom`, register
`hot-updater/babel-plugin` so Hot Updater can rewrite `overrideUri` to the
correct bundle path.
...
Once configured, the Babel plugin automatically handles `overrideUri` for all
DOM components. No additional code changes are required.
