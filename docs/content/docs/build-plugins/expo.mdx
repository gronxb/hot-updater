...
| ------------------ | --------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| `channel`          | `string`  | `"production"` | The release channel built into the app                                                                                                      |
| `verifyOnAppReady` | `boolean` | `false`        | Promote a new bundle when `notifyAppReady()` runs instead of on first content. See [Automatic Rollback](/docs/concepts/automatic-rollback#errors-caught-by-an-error-boundary) |
...
module.exports = function (api) {
  ...
  return {
    ...
    plugins: ['@hot-updater/expo/babel-plugin'], // [!code ++]
  };
};
```
