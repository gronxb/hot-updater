...
`hot-updater` has three components:
...
- Running `hot-updater deploy` asks the selected build plugin to generate a UUIDv7-formatted Bundle ID while bundling.
...
- Every new deployment generates a new Bundle ID.
