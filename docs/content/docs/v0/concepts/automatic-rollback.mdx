...
When you install a new bundle, Hot Updater first launches the bundle in a temporary state and keeps the last known-good bundle as a fallback. Startup must succeed before the new bundle is considered safe.

This happens automatically when you use `HotUpdater.wrap()`. For custom manual
flows without a root wrapper, call [`HotUpdater.init()`](/docs/v0/react-native-api/init)
