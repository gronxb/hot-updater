...
- Datadog site: set `DATADOG_SITE` to the site that issued the API key (for
  example, `DATADOG_SITE=datadoghq.eu`).
- Install the plugin:
...
<Callout title="Automatic Upload">
  When `withDatadog()` wraps your build plugin, all generated sourcemaps are
  uploaded to Datadog automatically during the `hot-updater deploy` process.
</Callout>
