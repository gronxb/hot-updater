---
title: "clearCrashHistory"
description: Clears the crash history to allow retrying previously failed bundles.
icon: Eraser
---

## Usage

Use `clearCrashHistory()` to reset the crash history and allow redeploying bundles that previously crashed.

```tsx
import { HotUpdater } from "@hot-updater/react-native";
import { Button, View } from "react-native";

function AdminPanel() {
  const handleClearHistory = () => {
    const result = HotUpdater.clearCrashHistory();

    if (result) {
      console.log("Crash history cleared successfully");
    } else {
      console.log("Failed to clear crash history");
    }
  };

  return (
    <View>
      <Button title="Clear Crash History" onPress={handleClearHistory} />
    </View>
  );
}

export default HotUpdater.wrap({
  baseURL: "<your-update-server-url>",
  updateStrategy: "appVersion",
})(AdminPanel);
```

## Return Value

Returns a boolean indicating success.

```typescript
clearCrashHistory(): boolean
```

- `true` - Crash history cleared successfully
- `false` - Failed to clear crash history

Clearing this list only allows those Bundle IDs to be attempted again. It does
not repair a failing Bundle, so clear it only after correcting the underlying
update or while testing recovery behavior.
