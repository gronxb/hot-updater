...
Use this to let Hot Updater manage automatic OTA updates.
...
- For a force update, the entry point remains blocked, and the progress updates as the bundle downloads.
...
Use a custom `resolver` instead of `baseURL` when you need control over network operations, such as a custom server, GraphQL, or complex authentication flows.
