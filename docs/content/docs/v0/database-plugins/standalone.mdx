...
<Callout type="info">
  ...
  routes are explicitly enabled and authorized. The canonical Channel-list
  endpoint is `GET /api/bundles/channels`.
</Callout>
...
| `enabled`                 | `"true" \| "false"`  | Exact boolean filter; omitted when unset                       |
| `idEq`                    | `string`             | Bundle ID equals this value                                    |
| `idGt`                    | `string`             | Bundle ID is greater than this value                           |
| `idGte`                   | `string`             | Bundle ID is greater than or equal to this value               |
| `idLt`                    | `string`             | Bundle ID is less than this value                              |
| `idLte`                   | `string`             | Bundle ID is less than or equal to this value                  |
| `idIn`                    | repeated `string`    | One query entry per Bundle ID                                  |
| `targetAppVersion`        | `string \| null`     | Literal `null` encodes a null filter                           |
| `targetAppVersionIn`      | repeated `string`    | One query entry per target app version                         |
| `targetAppVersionNotNull` | `"true" \| "false"`  | Filters nullability when set                                   |
| `fingerprintHash`         | `string \| null`     | Literal `null` encodes a null filter                           |
...
Undefined filters are omitted. `null` is sent as the literal string `null`,
booleans are sent as `true` or `false`, and array filters use repeated query
keys rather than comma-separated values. An empty array therefore emits no
entries for that filter. The repository may derive `page` from its internal
page-aligned offset when a management caller requests a numbered page.
