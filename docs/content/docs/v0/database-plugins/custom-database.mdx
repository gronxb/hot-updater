...
Use `createDatabasePlugin` to build and configure a plugin for your database provider.
...
The factory does not need to implement `updateBundle`, `appendBundle`, or `deleteBundle`. `createDatabasePlugin` generates and tracks these internally.

## Implementation Example

This custom database plugin uses a REST API:
...
The `createDatabasePlugin` fallback resolves cursor requests into `where.id`,
`orderBy`, and the internal `offset` before calling the provider. The REST
service must parse `where` and `orderBy`, apply all filters and ordering, and
only then apply `offset` and `limit`.
...
Generates pagination metadata for the internal offset window passed to your
provider:
