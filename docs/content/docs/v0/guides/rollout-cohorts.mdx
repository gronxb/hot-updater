...
Assume `HotUpdater.getCohort()` returns `411`.
...
The settings determine which devices receive the bundle:
...
Check whether cohort `411` is included:
...
After the app checks for updates and applies the eligible bundle, it shows:
...
`Target Cohorts` accepts both custom cohorts like `qa` and existing numeric
cohorts like `411` in the same list.
...
- [`HotUpdater.getCohort()`](/docs/v0/react-native-api/getCohort)
- [`HotUpdater.setCohort()`](/docs/v0/react-native-api/setCohort)
- [Deploy](/docs/v0/guides/deploy)
- [Console](/docs/v0/guides/console)
