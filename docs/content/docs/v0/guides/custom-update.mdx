...
check only after login, control updates based on app state, coordinate with navigation
...
This configures Hot Updater and notifies the update server that the current app
launch is ready.
</Callout>

## Check on Your Schedule

Call `checkForUpdate` when your app is ready to check for an update.
...
- When remote config or staged rollout settings allow it
...
- [`HotUpdater.init()`](/docs/v0/react-native-api/init)
- [`HotUpdater.wrap()`](/docs/v0/react-native-api/wrap)
- [`HotUpdater.checkForUpdate()`](/docs/v0/react-native-api/checkForUpdate)
- [`HotUpdater.reload()`](/docs/v0/react-native-api/reload)
- [`HotUpdater.addListener()`](/docs/v0/react-native-api/addListener)
- [`useHotUpdaterStore()`](/docs/v0/react-native-api/useHotUpdaterStore)
