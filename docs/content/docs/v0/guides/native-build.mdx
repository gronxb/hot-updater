...
Use Hot Updater's native commands to create store-ready artifacts or launch the app on a device from the CLI. `build:android`, `build:ios`, `run:android`, and `run:ios` reuse the same configuration defined in `hot-updater.config.ts` and respect your update strategy. The `build:*` commands copy artifacts into a predictable output directory, while the `run:*` commands build, install, and launch directly.
...
- `updateStrategy: "fingerprint"` regenerates `fingerprint.json` before the build and shows changes since the last checked-in fingerprint.
