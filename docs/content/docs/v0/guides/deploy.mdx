...
1. Configure `hot-updater.config.ts`.
2. Wrap your components using `HotUpdater.wrap`.
3. Include the native code.

If you haven't completed these steps, refer to the [Supabase Provider](/docs/v0/managed/supabase) guide.
...
Options:
  ...
  -d, --disabled                               create the update disabled (default: false)
...
See [Bundle Diffing](/docs/v0/guides/bundle-diffing) for the runtime behavior and
fallback rules.
...
The `-c` (or `--channel`) flag sets the release channel. The default is `production`.

For more details about default channels, refer to
[Default Channel Setup](/docs/v0/guides/channel-management).
...
the console, see [Rollout & Cohorts](/docs/v0/guides/rollout-cohorts).
