...
Fingerprint detects native code changes and blocks incompatible OTA updates.
...
metadata. `stringResourcePaths` remains available only as a legacy read
fallback for projects that already use `hot_updater_fingerprint_hash` in
`strings.xml`.

The default paths work for most projects.
