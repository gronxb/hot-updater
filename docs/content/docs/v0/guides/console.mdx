...
Configure console settings in `hot-updater.config.ts`. These settings do not
affect the bundle. Both `storage` and `database` plugins are required.
...
After deleting bundle records, use the [Storage Cleanup](/docs/v0/guides/storage-cleanup)
...
With the git URL configured, each commit hash links to its commit in your repository.
