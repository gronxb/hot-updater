...
<Tabs items={['React Native CLI', 'Expo']}>
  <Tab value="React Native CLI">
    ...
    Export the public key to native configuration files manually.
  </Tab>
  <Tab value="Expo">
    ```bash
    # 1. Generate keys
...
Expo projects do not need to run `npx hot-updater keys export-public` manually.
...
> For local EAS builds (`eas build --local`), this avoids configuring environment variables.
