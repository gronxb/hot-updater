...
| --------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `node`    | `hot-updater deploy`, `hot-updater bundle promote --action copy`, bundle diff generation, and console server actions  | Local file upload/download, existence checks, and delete      |
...
- `node`: writes, checks, and materializes files in deploy-time tooling.
...
| Existence policy    | Provider metadata/head request                 | Return `false` only when the object is missing; rethrow other failures.  |
| Runtime URL policy  | Signed URL, public CDN, or authenticated route | `runtime.getDownloadUrl` must return HTTP(S).                            |
  ...
  factory: (config) => ({
    node: {
      ...
      async exists(storageUri) {
        // Return false only for your provider's missing-object response.
        return storageClient.exists(storageUri);
      },
...
Checks whether a stored object can be safely reused by deploy.

```typescript
exists: (storageUri: string) => Promise<boolean>;
```

Use a provider metadata or head request when available. Return `false` only for
the provider's missing-object response. Permission errors, invalid storage URIs,
provider outages, and other failures must be rethrown so deploy does not mistake
an inaccessible object for an absent one.
...
Install the SDK packages imported by the example as direct dependencies:
...
Use your custom plugin in `hot-updater.config.ts`. This excerpt shows only the
storage field; keep the required `build`, `database`, and `updateStrategy`
fields in the complete project configuration.
...
The configured plugin must implement the complete `node` profile because
`hot-updater` CLI commands upload, check, download, and delete stored files.
...
reads. Import the database adapter configured for your server as well; it is a
required `createHotUpdater` option.
...
- `exists` returns `true` for an existing object and `false` for a missing one.
- `exists` rethrows permission, connectivity, and other provider failures.
...
- `readText` returns text for an existing metadata object and `null` for a
  missing one.
- `readText` rethrows permission, connectivity, and malformed provider
  responses.
