...
A storage plugin tells Hot Updater how to save bundle artifacts, read small
  ...
  factory: (config) => ({
    node: {
      ...
      async exists(storageUri) {
        return config.objectExists(storageUri);
      },
...
expected provider and bucket before touching storage. This check prevents
...
provider outages, and other failures must be rethrown.
...
bundle diff generation, and console server actions that upload, check, download,
or delete local files. The factory returns the `node` profile directly.

```typescript
import { createNodeStoragePlugin } from "@hot-updater/plugin-core";

interface NodeOnlyConfig {
  objectExists(storageUri: string): Promise<boolean>;
}
...
Create the storage value used by your complete `hot-updater.config.ts`:
...
Assign this value to the required `storage` field alongside the config's
`build`, `database`, and `updateStrategy` fields. The configured plugin must
implement the `node` profile because CLI commands upload, check, download, and
delete local files.
