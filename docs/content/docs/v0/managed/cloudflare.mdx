...
- `hot-updater.config.ts`: Configures Cloudflare integration for your project.
...
The persisted `HOT_UPDATER_CLOUDFLARE_R2_PRIVATE` value replays the privacy
choice on later init runs.
...
<Callout type="info">
The managed Cloudflare runtime exposes the public update-check endpoints and `/version`. Internally, it is equivalent to `createHotUpdater({ routes: { updateCheck: true, bundles: false } })`, so bundle management endpoints are not mounted on the public worker URL.
</Callout>
