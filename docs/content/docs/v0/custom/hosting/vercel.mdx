...
Pull the target environment and apply the schema before deployment:
...
Set `HOT_UPDATER_AUTH_TOKEN`, `R2_ENDPOINT`, `R2_ACCESS_KEY_ID`,
...
<Callout type="warn">
The `storage` plugin must match the `storages` in your server's `createHotUpdater`. Configure `HOT_UPDATER_AUTH_TOKEN` in every Vercel environment and in the CLI environment.
</Callout>
