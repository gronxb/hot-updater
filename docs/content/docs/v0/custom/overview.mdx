...
the CLI packages in the React Native project.
...
- [Prisma](./database/prisma) - ORM with data modeling
...
Generate and apply the adapter's schema before serving requests. Kysely and
MongoDB use `hot-updater db migrate`, Drizzle applies the generated schema with
Drizzle Kit, and Prisma applies the merged schema with `prisma db push`. S3 does
...
- **Cloud storage**: Use existing AWS S3, Cloudflare R2, Supabase, or Firebase storage
- **Type safety**: Full TypeScript support across all adapters
- **Operational control**: Add framework-specific lifecycle and error handling for your deployment
