...
standalone plugin in the React Native project that runs the CLI.
...
download operations. With R2 S3-compatible credentials, the root `r2Storage`
plugin supports both the `node` and `runtime` profiles. Edge deployments can
still use a runtime implementation backed by a platform binding.
...
```typescript title="src/hotUpdater.ts (routes excerpt)"
export const hotUpdater = createHotUpdater({
  database: kyselyAdapter({ db, provider: "sqlite" }),
  storages: [
    r2Storage({
...
into `prisma/schema.prisma` for Prisma Migrate. S3 does not require a schema.
