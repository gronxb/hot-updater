...
With `routes: { updateCheck: true, bundles: true }`, the server mounts the v0
management contract used by `standaloneRepository`:
...
These six routes can be remapped with the `routes` option shown above. They do
not authenticate themselves; protect `/hot-updater/api/*` before mounting the
handler.
