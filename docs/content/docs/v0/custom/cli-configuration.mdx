...
`fingerprintHash`, and `limit`/`page`/`after`/`before`. Undefined filters are
omitted; nullable filters use the literal `null`, booleans use `true` or
`false`, and array filters use repeated query keys rather than comma-separated
values.
