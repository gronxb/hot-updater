...
project root, so run their commands with the project root as the working
directory.
...
<Callout type="warn">
  ...
  `createHotUpdater`. Both use `s3Storage` in this example. Because
...
See the [standalone List Bundles contract](/docs/v0/database-plugins/standalone#list-bundles-contract)
for the complete query encoding and response requirements.
