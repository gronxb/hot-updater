...
<Callout type="warn">
Keep deployment credentials from `.env.hotupdater` out of app source and build configuration. Configure public app values, such as the update server URL, explicitly. See the [Security guidelines](/docs/v0/policy/security#plugin-token-protection).
</Callout>
...
Configure bundling through the `build` field in `hot-updater.config.ts`.
