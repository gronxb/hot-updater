...
2. **Provider Selection**: Select from available providers:
   - [Supabase](/docs/v0/managed/supabase)
   - [Firebase](/docs/v0/managed/firebase)
   - [Cloudflare](/docs/v0/managed/cloudflare)
   - [AWS](/docs/v0/managed/aws)
...
<Callout type="info">
For detailed provider-specific setup instructions, see the [Self-Hosting (Managed)](/docs/v0/managed/supabase) documentation.
</Callout>

<Callout type="warn">
The environment variables in `.env.hotupdater` are used only during deployment, not in your app bundle. However, if you're using `react-native-dotenv`, review the [Security guidelines](/docs/v0/policy/security#plugin-token-protection).
</Callout>
...
<Callout>
For Expo projects, the config plugin automatically generates the native code configuration described in the next section.
</Callout>

<Callout type="info">
If you use Expo DOM components (`use dom`), see [Expo DOM Components (`use dom`)](/docs/v0/build-plugins/expo#expo-dom-components-use-dom) for the additional Babel plugin setup.
</Callout>
...
- [App Version Strategy](/docs/v0/guides/update-strategies/app-version) - Match updates to the native app version
- [Fingerprint Strategy](/docs/v0/guides/update-strategies/fingerprint) - Opt in to native dependency fingerprints
- [Default Channel Setup](/docs/v0/guides/channel-management) - Configure the app's default release channel
- [Runtime Channel Switch](/docs/v0/guides/runtime-channel-switch) - Temporarily move a device to another channel without rebuilding
- [Simulator Testing](/docs/v0/guides/simulator-test) - Test updates in release mode before production
- [Security Best Practices](/docs/v0/policy/security) - Secure your update infrastructure
