...
Storage is configured with a plugin and the database with an adapter in `hot-updater.config.ts`. The Server is initialized and deployed using the `hot-updater init` command.
