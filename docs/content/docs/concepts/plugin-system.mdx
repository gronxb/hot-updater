...
Storage plugins expose storage operations directly:

- deploy-time APIs for upload, delete, and local file downloads.
- update-check APIs for HTTP(S) download URLs and small metadata reads.

`hot-updater.config.ts` and `createHotUpdater` use the same operation names.
Most apps can use the official storage plugins directly; custom plugin authors
only need to implement the operations their provider can support.
