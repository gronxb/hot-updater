...
- **`@hot-updater/bare`**: Runs the project's configured bundler through React Native CLI's `bundle` command.
- **`@hot-updater/rock`**: Runs the project's configured bundler through the `bundle` command in [Rock](https://github.com/callstack/rock) CLI (formerly RNEF).
