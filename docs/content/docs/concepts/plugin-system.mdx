...
Build plugins handle the bundling/native building process when you run the `hot-updater deploy` or `hot-updater build:(android|ios)` commands. They generate the JavaScript bundles for your updates or native build artifacts for your app.

The basis of native build processes are shared between all plugins. They are not much than a wrapper of Gradle and xcodebuild cli but include some extra logic to handle the native build process for each platform.
