...
`s3Storage` at `@hot-updater/aws/lambda`. Supabase Edge factories move from
their `supabaseEdgeFunction*` names to `supabaseDatabase` and
`supabaseStorage` at `@hot-updater/supabase/edge`.
