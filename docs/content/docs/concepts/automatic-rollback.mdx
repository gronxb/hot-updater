...
3. **Verification**:
   - If React content appears successfully, native code automatically verifies the launch
...
`notifyAppReady()` is now informational only. It reads the current launch result for optional JS callbacks, but rollback and promotion are decided entirely on the native side.
