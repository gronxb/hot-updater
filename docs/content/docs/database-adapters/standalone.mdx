...
> bundle routes, not a fixed-model database adapter. It forwards channel-list
> reads to the existing channels endpoint while bundle rows keep their channel
> name directly in `channel`.
