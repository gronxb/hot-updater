---
title: "Datadog"
description: "The `withDatadog()` adapter for Hot Updater enables automatic sourcemap upload to Datadog during the update bundle build process."
icon: datadog
---
...
## Step 1: Wrap Your Build Adapter

Use `withDatadog()` to wrap any compatible build adapter such as bare.
...
When `withDatadog()` wraps your build adapter, all generated sourcemaps are uploaded to Datadog automatically during the `hot-updater deploy` process.
</Callout>
