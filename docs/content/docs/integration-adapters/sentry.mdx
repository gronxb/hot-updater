---
title: "Sentry"
description: "The `withSentry()` adapter for Hot Updater enables automatic sourcemap upload to Sentry during the update bundle build process."
icon: sentry
---
...
## Step 2: Wrap Your Build Adapter

Use `withSentry()` to wrap any compatible build adapter such as bare.
...
When `withSentry()` wraps your build adapter, all generated sourcemaps are uploaded to Sentry automatically during the `hot-updater deploy` process.
</Callout>
