...
Most projects do not need an extra Hot Updater bundler plugin. The only
...
Add `hot-updater/babel-plugin` only if your Expo app uses DOM components with
`use dom` (typically Expo SDK < 55).

Run the following command to customize your Babel configuration when that
applies:
...
<Callout>
For Expo projects without `use dom`, you can skip the Babel plugin entirely.
</Callout>
