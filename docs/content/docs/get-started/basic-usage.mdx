...
Hot Updater now configures bundling through the `build` field in `hot-updater.config.ts`. The old `hot-updater/babel-plugin` and `@hot-updater/repack` setup is no longer used.
