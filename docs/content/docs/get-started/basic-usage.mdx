...
<Callout type="warn">
The environment variables in `.env.hotupdater` are used only during deployment, not in your app bundle. However, if you're using `react-native-dotenv`, review the [Security guidelines](/docs/policy/security#plugin-token-protection).
</Callout>
...
<Callout type="info">
If you use Expo DOM components (`use dom`), see [Expo DOM Components (`use dom`)](/docs/build-plugins/expo#expo-dom-components-use-dom) for the additional Babel plugin setup.
</Callout>
...
<Tabs items={['Kotlin (RN 0.82+)', 'Kotlin (RN 0.76–0.81)', 'Kotlin (RN ≤0.75)', 'Java']}>
...
For React Native 0.76 through 0.81, add this override inside the generated `DefaultReactNativeHost`. Keep the loader and host setup generated by your React Native version:

```kt title="android/app/src/main/java/com/<your-app-name>/MainApplication.kt"
import com.hotupdater.HotUpdater // [!code ++]

// Inside the generated DefaultReactNativeHost:
...
For React Native 0.75 and below, add the same override inside the generated `ReactNativeHost`. Keep the older template's loader and host setup:

```kt title="android/app/src/main/java/com/<your-app-name>/MainApplication.kt"
import com.hotupdater.HotUpdater // [!code ++]

// Inside the generated ReactNativeHost:
override fun getJSBundleFile(): String? { // [!code ++]
  return if (BuildConfig.DEBUG) null // [!code ++]
  else HotUpdater.getJSBundleFile(applicationContext) // [!code ++]
} // [!code ++]
...
For React Native 0.79 and above, keep the generated `RCTReactNativeFactory` setup and change bundle selection in its `ReactNativeDelegate`:
...
For React Native 0.78 and below, modify the generated `RCTAppDelegate` subclass:
...
- [Fingerprint Strategy](/docs/guides/update-strategies/fingerprint) - Target updates by native fingerprint
