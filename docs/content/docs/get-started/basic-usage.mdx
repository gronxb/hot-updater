...
Before configuring the build adapter, you need to set up a provider to handle updates. Hot-updater supports multiple managed providers that handle storage and update distribution.
...
1. **Build Adapter Selection**: Choose your build system (Bare/React Native CLI, Rock, or Expo)
...
The example below uses the Bare (React Native CLI) build adapter. If you selected Rock or Expo, only the `build` field changes as shown in the build configuration section below.
...
<Callout type="warn">
The environment variables in `.env` are used only during deployment, not in your app bundle. However, if you're using `react-native-dotenv`, review the [Security guidelines](/docs/policy/security#adapter-token-protection).
</Callout>
...
Hot Updater now configures bundling through the `build` field in `hot-updater.config.ts`. Legacy build-time adapter examples are no longer used.

<Tabs items={['Bare (React Native CLI)', 'Rock', 'Expo']}>
  <Tab value="Bare (React Native CLI)">

Use the Bare build adapter for standard React Native CLI projects:
...
Use the Rock build adapter for Rock projects:
...
Use the Expo build adapter in `hot-updater.config.ts`:
...
<Callout type="info">
If you use Expo DOM components (`use dom`), see [Expo DOM Components (`use dom`)](/docs/build-adapters/expo#expo-dom-components-use-dom) for the additional Babel plugin setup.
</Callout>
