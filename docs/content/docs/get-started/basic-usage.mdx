...
#### Provide all initialization inputs from a file

To avoid prompts, put the initialization choices and provider values in a
separate dotenv file:

```bash title=".hot-updater-init.env"
...
```package-install
npx hot-updater init --env-file .hot-updater-init.env
```

When `--env-file` is present, `init` does not open configuration prompts. It
reports all missing provider values and exits instead. Process environment
variables take precedence over this file, and this file takes precedence over
values already present in `.env.hotupdater`.

The input file is never modified and must be separate from the managed
`.env.hotupdater` output. Keep it out of version control and delete it after
initialization if it contains temporary secrets.
