...
description: Create your own storage adapter for any storage provider
icon: Hammer
---
...
Build a custom storage adapter when the built-in providers do not match your
storage backend.
...
storage adapter should not know about database rows, rollout rules, or update
selection.

Storage adapters are split into environment profiles. The profile name describes
...
Use the helper that matches where the adapter will run:

| Helper                         | Use when                                                                                                                       |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| `createNodeStoragePlugin`      | The adapter is configured in `hot-updater.config.ts` and runs from CLI/Node workflows such as `hot-updater deploy`.             |
| `createRuntimeStoragePlugin`   | The adapter is passed to `createHotUpdater({ storages })` from `@hot-updater/server` for update-check runtime reads.            |
| `createUniversalStoragePlugin` | The same adapter should work in both places: `hot-updater.config.ts` for CLI commands and `createHotUpdater` for update checks. |

Most providers should use `createUniversalStoragePlugin`. Split providers, such
as Cloudflare R2, can expose one `node` adapter for Wrangler-based upload and
download, and a separate `runtime` adapter for Worker bindings.

## What Universal Means

A universal storage adapter supports both sides of Hot Updater:
...
the adapter can provide both profile contracts from the configuration it
receives. If your provider uses different APIs in Node and edge runtimes, keep
them as separate `node` and `runtime` adapters instead.
...
| `supportedProtocol` | `custom-s3`                                  | Must match every `storageUri` the adapter returns.                        |
...
Use `createUniversalStoragePlugin` when one adapter can be used in both
...
This example implements a universal S3-compatible storage adapter. The same
...
Use `createNodeStoragePlugin` when the adapter only needs to run from CLI/Node
...
Use `createRuntimeStoragePlugin` when the adapter only needs to run inside
...
Use your custom adapter in `hot-updater.config.ts`.
...
The configured adapter must implement the `node` profile because `hot-updater`
CLI commands upload, download, and delete local files.

## Custom Server Usage

Use the same universal adapter with `createHotUpdater` from
`@hot-updater/server`. The adapter passed to `storages` must implement the
...
- `readText` returns text for an existing metadata object and `null` for a
  missing one.
- The adapter factory is not initialized until a profile method is accessed.
