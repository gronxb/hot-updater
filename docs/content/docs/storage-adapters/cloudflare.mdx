...
For a single adapter that supports both deploy-time storage and runtime presigned
URLs, use the S3-compatible AWS adapter. See the [AWS S3 Storage
documentation](/docs/storage-adapters/aws) for more details.
...
`r2Storage` is a CLI/deploy-time storage adapter. It implements the `node`
...
The Worker runtime adapter implements the `runtime` profile. It signs bundle
...
- For a single adapter that supports both deploy-time and runtime profiles, use
