...
export const bundle_events = sqliteTable(
  ...
  (table) => [
    ...
    index("bundle_events_user_id_idx").on(
      table.user_id,
      table.received_at_ms,
      table.id,
    ),
    index("bundle_events_username_idx").on(
      table.username,
      table.received_at_ms,
      table.id,
    ),
...
export const client_access_keys = sqliteTable(
  "client_access_keys",
  {
    id: text("id", { length: 255 }).primaryKey().notNull(),
  ...
  (table) => [
    ...
    index("client_access_keys_created_at_idx").on(
      table.created_at_ms,
      table.id,
    ),
  ],
...
export const private_hot_updater_settings = sqliteTable(
  ...
  {
    id: text("id", { length: 255 }).primaryKey().notNull(),
    version: text("version", { length: 255 }).notNull().default("0.37.0"),
  },
);
