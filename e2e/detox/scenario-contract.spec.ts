...
describe("Detox scenario contract", () => {
  ...
  it("reattaches focused and restarted Android apps through Detox", async () => {
    ...
    // waits for a stable replacement process before reconnecting once; the
    // launcher may remain focused until Detox reconnects.
    ...
    expect(controllerSource).toContain(
      "E2E_ANDROID_RESTART_STABLE_OBSERVATIONS",
    );
    expect(controllerSource).toContain("if (lastProcessId) {");
    expect(controllerSource).not.toContain(
      "lastFocusedPackage === fixtureSession.appId && lastProcessId",
    );
