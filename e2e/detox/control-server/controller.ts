...
async function waitForAndroidRestart(signal?: AbortSignal) {
  ...
  for (
    ...
    if (lastFocusedPackage === fixtureSession.appId && lastProcessId) {
      if (lastProcessId === stableProcessId) {
        stableObservations += 1;
      } else {
        stableProcessId = lastProcessId;
        stableObservations = 1;
      }

      if (stableObservations >= E2E_ANDROID_RESTART_STABLE_OBSERVATIONS) {
        logDetoxFixture("android replacement process ready", {
          attempt,
  ...
  throw new Error(
    `Timed out waiting for Android replacement process; focusedPackage=${String(lastFocusedPackage)}, processId=${lastProcessId || "none"}`,
  );
}
