...
export const lynxRuntimeChannelSwitchResetScenario = {
  name: "runtime-channel-switch-reset",
  run: async (app: LynxAppDriver) => {
    await app.control(
    ...
    await app.control(
      ...
      },
      {
        ...
        saveResultFieldsAs: { releaseId: "runtimeReleaseId" },
      },
    );
    ...
    const beforeReset = assertManagedMainPage(
      await app.captureGenerationEvents(
        "capture runtime channel generation before reset",
      ),
      {
        bundleId: app.readScenarioString("runtimeBundleId"),
        releaseId: app.readScenarioString("runtimeReleaseId"),
      },
    );
    ...
    assertManagedTransition(
      ...
      ),
      {
        before: beforeReset,
        after: {
          bundleId: app.readScenarioString("builtInBundleId"),
          releaseId: null,
        },
      },
    );
    ...
    await app.assertText("assert reset launch status", "launch-status-result", [
      "Current Launch Status: UNCHANGED",
      "Current Launch Status: UPDATE_APPLIED",
    ]);
    await app.assertText(
