...
The local PEM configuration shape and opt-in behavior remain compatible with
v0. No signing plugin or separate public key file is required:
...
fallbacks. Console inspection never reads the private key: without an explicit
public path, it looks for a sibling `public-key.pem`. A missing public file
makes inspection unavailable, but does not prevent local signing.
...
- Deploy signing requires RSA keys of at least 2048 bits and verifies every
  ...
  (`BEGIN PUBLIC KEY`). Keys generated by the v0 CLI already meet these
  requirements; weaker or unsupported keys must be replaced through a native
  key rollout.
- An explicit `publicKeyPath` is authoritative. A missing, invalid, or
  mismatched file fails deployment instead of falling back to another key.
  Deploy also rejects invalid or mismatched keys in native configuration files
  when those files are present; it cannot inspect already installed binaries.
...
- `keys export-public` now defaults to cancelling a different or invalid
  embedded-key replacement. `--yes` explicitly acknowledges that rotation.

The optional `remoteSigning`, `awsKmsSigning`, and `googleCloudKmsSigning`
plugins from `hot-updater/signing` require `publicKeyPath`; the built-in local
config does not. Signing remains independent of storage and database choice.
...
Local signing retains the v0 `enabled`/`privateKeyPath` configuration described
above. Separately, persisted runtime compatibility is limited to state that
can remain on a device when a new v1 native build is installed over a v0 app:
...
5. If signing is enabled, retain the v0 local config or explicitly select a
   signing plugin. Check the key requirements and native public-key match;
   plan a native-first rollout if the signing key changes.
