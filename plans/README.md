...
| M2 | 내장 resolver·lazy 인덱스 | M0, M1 | IN PROGRESS | Android unit, Swift 28 tests, iOS Release build; device E2E pending |
| M3 | 전송 비용 검증·아카이브 제거 | M0–M2 gate | IN PROGRESS | archive code/schema removed; Release device measurement pending |
...
- 2026-09-20: artifact protocol v1을 별도 `/artifacts/v1/...` endpoint로 확정했다. v1은 모든 목표 파일의 원본 descriptor를 제공하고 archive URL/hash를 사용하지 않는다. legacy endpoint 의미는 유지한다.
...
- 2026-09-20: Detox default suite가 양 플랫폼에서 `bspatch-builtin-to-diff-ota`를 포함하는 dry-run을 통과했다. 실제 Release device E2E는 남아 있다.
