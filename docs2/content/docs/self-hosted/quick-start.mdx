...
<Callout type="warn">
The `storage` plugin here must match the `storagePlugins` in your server's `createHotUpdater` configuration. Both use `s3Storage` in this example.
</Callout>
...
- Explore other [Database Adapters](./database) (Drizzle, Prisma, MongoDB)
- Try different [Server Frameworks](./frameworks) (Express, Elysia)
