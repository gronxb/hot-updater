...
Add the key-value helper to the unstable `@hot-updater/server/database` subpath, for the DynamoDB and Firestore stores that follow. `createKvAdapter({ store, tablePrefix })` implements the storage adapter over a `KeyValueStore`, which provides strongly consistent point reads, one partition's sort-key range per page, and atomic writes whose conditions see the state before the write.
