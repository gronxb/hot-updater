...
- **Admin API protocol 2:** `/version` reports `adminProtocol: 2` and is also served by the admin handler. The list routes take `v=2` for keyset pages (`cursor`, `limit`, `order`) and the indexed filter sets. New routes: `POST /releases`, `POST /releases/:id/promote`, `POST /release-catalogs/:scopeKey/preflight`, `POST /bundles/delete`, `GET /bundles/:id/children`, and `GET /base-candidates/:candidateKey`. Protocol 1 routes stay until 1.0.
...
- **`hot-updater keys`:** when the config has plugins, it uses `hotUpdater.api.apiKeys`. It refuses plugins without `apiKeys()`.
