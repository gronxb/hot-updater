// allow: SIZE_OK — existing editor owns dense bundle form state; G004 only gates actions.
import {
...
import { isBundleDownloadSupported } from "@/lib/console-capabilities";
...
export function BundleEditorForm({
  ...
  const updateCapability = capabilities?.updateBundle;
  const promoteMoveCapability = capabilities?.promoteBundleMove;
  const promoteCopyCapability = capabilities?.promoteBundleCopy;
  const deleteCapability = capabilities?.deleteBundle;
  const canUpdate = updateCapability?.supported === true;
  const canPromote =
    promoteMoveCapability?.supported === true ||
    promoteCopyCapability?.supported === true;
  const canDelete = deleteCapability?.supported === true;
  const canDownload = capabilities
    ? isBundleDownloadSupported(bundle, capabilities)
    : false;
  const loadingCapabilityReason = capabilities
    ? undefined
    : "Loading console capabilities";
  const updateDisabledReason =
    updateCapability?.reason ?? loadingCapabilityReason;
  const promoteDisabledReason =
    promoteMoveCapability?.reason ??
    promoteCopyCapability?.reason ??
    loadingCapabilityReason;
  const deleteDisabledReason =
    deleteCapability?.reason ?? loadingCapabilityReason;
  const downloadDisabledReason =
    capabilities?.downloadBundle.reason ?? loadingCapabilityReason;
  const isUpdateDisabled = !canUpdate || isSaving;
    ...
    <div className="space-y-6">
      ...
      >
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={isUpdateDisabled || !newCohort.trim()}
        ...
        <div className="pt-2">
          <Button
            ...
            className="w-full"
            disabled={
              !hasChanges || isSaving || hasTargetAppVersionError || !canUpdate
            }
            title={!canUpdate ? updateDisabledReason : undefined}
          >
      ...
      <div className="space-y-4">
        ...
        <Button
          ...
          disabled={isSaving || !canPromote}
          title={!canPromote ? promoteDisabledReason : undefined}
        >
          Promote to Channel
        </Button>
        ...
        <Button
          ...
          title={!canDownload ? downloadDisabledReason : undefined}
        >
