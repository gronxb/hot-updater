...
-- noqa: SIZE_OK - atomic migration preserves data, RLS/policies, marker order.
...
BEGIN
  ...
  IF shape IN ('v1', 'v2') AND NOT (
    SELECT relrowsecurity
    FROM pg_class
    WHERE oid = to_regclass('public.bundle_events')
  ) THEN
    RAISE EXCEPTION 'Supabase Analytics adoption requires row level security';
  END IF;
