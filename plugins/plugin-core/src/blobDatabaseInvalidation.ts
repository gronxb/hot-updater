...
const bundlePaths = (
  ...
  if (!bundle.fingerprint_hash && !bundle.target_app_version) return [];
  assertBlobUpdateRouteSegment("channel", bundle.channel);
  const paths: string[] = [];
  if (bundle.fingerprint_hash) {
    assertBlobUpdateRouteSegment("fingerprintHash", bundle.fingerprint_hash);
    paths.push(
