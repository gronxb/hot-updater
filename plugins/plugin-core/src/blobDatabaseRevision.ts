...
const pointerUnknownFields = new WeakMap<
  BlobDatabasePointer,
  readonly string[]
>();
const pointerFields = new Set(["version", "active_revision"]);

export const createBlobDatabasePointer = (
  revision: string,
): BlobDatabasePointer => ({ version: 2, active_revision: revision });

export const assertBlobDatabasePointerCompatible = (
  pointer: BlobDatabasePointer,
): void => {
  const fields = pointerUnknownFields.get(pointer);
  if (fields && fields.length > 0) {
    throw new BlobDatabaseUnknownFieldsError(fields, "revision pointer");
  }
};
...
export const parseBlobDatabasePointer = (
  ...
  const pointer = createBlobDatabasePointer(revision);
  const unknownFields = Object.keys(input)
    .filter((key) => !pointerFields.has(key))
    .sort((left, right) => left.localeCompare(right));
  if (unknownFields.length > 0) {
    pointerUnknownFields.set(pointer, unknownFields);
  }
  return pointer;
};
