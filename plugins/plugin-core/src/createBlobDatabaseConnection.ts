// noqa: SIZE_OK - Existing blob database provider core; splitting belongs to a dedicated provider cleanup.
...
import { setBundleStoreReadHint } from "./databaseBundleResources";
import type {
...
const BLOB_API_BASE_PATH = "/api/check-update";

export const createBlobDatabaseConnection = ({
  listObjects,
  loadObject,
  uploadObject,
  deleteObject,
  ...
  invalidatePaths,
}: DatabasePluginBlobOperations): DatabasePluginResourceDeclaration => {
  ...
  const addAppVersionInvalidationPaths = (
    ...
    if (!isExactVersion(targetAppVersion)) {
      pathsToInvalidate.add(`${BLOB_API_BASE_PATH}/app-version/${platform}/*`);
      return;
    }
    ...
    for (const version of normalizedVersions) {
      pathsToInvalidate.add(
        `${BLOB_API_BASE_PATH}/app-version/${platform}/${version}/${channel}/*`,
      );
    }
  };
  ...
  const addLookupInvalidationPaths = (
    ...
    if (fingerprintHash) {
      pathsToInvalidate.add(
        `${BLOB_API_BASE_PATH}/fingerprint/${platform}/${fingerprintHash}/${channel}/*`,
      );
      return;
    }
  ...
  const findBundleRecords = async () =>
    (await loadAllBundlesForManagementFallback()).map(toDatabaseBundleRecord);
  ...
  const replaceBundlePatches = async (
    ...
    await flushChangedBundles([
      {
        operation: "update",
        data: toBlobBundleReadModel(toDatabaseBundleRecord(current), patches),
      },
    ]);
  };
  ...
  const flushChangedBundles = async (
    ...
    await forEachWithConcurrency(
      ...
      STORAGE_OPERATION_CONCURRENCY,
      async (key) => {
        const currentBundles = (await loadOptionalObject<Bundle[]>(key)) ?? [];
  ...
  const bundles = setBundleStoreReadHint(
    ...
    async ({ where }) =>
      (await loadAllBundlesForManagementFallback(where)).map(
        toDatabaseBundleRecord,
      ),
  );
