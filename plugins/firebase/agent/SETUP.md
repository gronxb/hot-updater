...
2. **Service readiness.** Verify required billing, Cloud Functions/Cloud Build/
   Artifact Registry and IAM Service Account Credentials APIs, default Firestore database, and the actual default
...
6. **Download signing.** Read the deployed function's actual runtime service
   account; do not assume the default account name. Besides Storage access, it
   needs iam.serviceAccounts.signBlob on the runtime service account itself to
   generate download URLs. Reuse a working grant, or add roles/iam.serviceAccountTokenCreator
   for that runtime identity on the signing account itself. Interactive init's
   project-level grant also satisfies this requirement. Preserve unrelated IAM
   bindings and verify the IAM Service Account Credentials API is enabled.
   See [Google Cloud signing requirements](https://cloud.google.com/storage/docs/authentication/creating-signatures).
7. **Local access and key.** Complete COMMON.md with project ID, region, actual
...
8. **Verify.** Check the direct function URL's /ping, /version and authenticated
   client requests. Record actual project/function/region and deployed version.
   Verify signing access for the actual runtime identity, and resolve/download
   an existing artifact when available. A public /version or empty catalog check
   does not exercise download signing. Inspect logs and retry only failed steps
