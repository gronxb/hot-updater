...
const createHarness = async (): Promise<InsightsModelConformanceHarness> => {
  const instanceId = harnessId++;
  const projectId = `firebase-insights-conformance-${instanceId}`;
  const namespaceSuffix = (instanceId * 2 + 1).toString(16).padStart(12, "0");
  const otherNamespaceSuffix = (instanceId * 2 + 2)
    .toString(16)
    .padStart(12, "0");
  const databaseNamespace = `10000000-0000-4000-8000-${namespaceSuffix}`;
  const otherDatabaseNamespace = `10000000-0000-4000-8000-${otherNamespaceSuffix}`;
  const app = initializeApp({ projectId }, projectId);
  const otherProjectId = `${projectId}-other`;
  const otherApp = initializeApp({ projectId: otherProjectId }, otherProjectId);
  apps.push(app, otherApp);
  const firestore = getFirestore(app);
  const otherFirestore = getFirestore(otherApp);
  const collections = createFirebaseInsightsCollections(
    firestore,
    databaseNamespace,
  );
  const otherCollections = createFirebaseInsightsCollections(
    otherFirestore,
    otherDatabaseNamespace,
  );
  const sourceVector = [
    ...Array.from(
      { length: FIREBASE_INSIGHTS_SOURCE_SHARDS },
      (_, shard) => [`live_${shard.toString(16).padStart(2, "0")}`, 0] as const,
    ),
    ["legacy", 0] as const,
  ];
  await Promise.all(
    [collections, otherCollections].flatMap((namespaceCollections) => [
      namespaceCollections.control.doc("layout").set({
        ...
        databaseNamespace: namespaceCollections.databaseNamespace,
      }),
