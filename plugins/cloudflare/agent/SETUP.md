...
2. **Worker inputs.** Install worker/package.json dependencies in worker/.
   Fill worker/wrangler.json with the account ID, Worker name, D1 ID/name, and
   R2 bucket before running Wrangler commands. Keep bindings DB and BUCKET,
   variable BUCKET_NAME, and the supplied cache configuration and compatibility
   date. The bundled worker/dist/index.js is the runtime used by interactive init.
3. **Schema.** Replace %%BUCKET_NAME%% in worker/migrations with the selected
   bucket name. Inspect D1 migration history, then apply pending files in order.
   With Wrangler, work in the configured worker/ directory and run
