...
CREATE TABLE releases (
  ...
  CONSTRAINT releases_rollout_cohort_count_check CHECK (
    rollout_cohort_count BETWEEN 0 AND 1000
  ),
...
CREATE INDEX bundle_events_install_idx
  ON bundle_events(install_id, type, received_at_ms, id);
CREATE INDEX bundle_installations_user_id_idx
  ON bundle_installations(user_id, install_id);
CREATE INDEX bundle_installations_received_at_idx
  ON bundle_installations(received_at_ms);
