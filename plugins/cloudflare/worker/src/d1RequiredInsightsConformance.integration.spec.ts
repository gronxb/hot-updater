  ...
  async (): Promise<RequiredInsightsModelConformanceHarness> => {
    ...
    const runTargetedStep = async (
      ...
      if (input.maxRequests < 3) {
        return createD1InsightsMaintenance(executor).runStep(input);
      }
      await database
        .prepare(
    ...
    const createHarness = (): RequiredInsightsModelConformanceHarness => ({
      model: expiredModel(
        createD1RequiredInsightsModel(
          primaryExecutor,
          "d1-conformance-primary",
        ),
        expired,
      ),
      otherNamespaceModel: createD1RequiredInsightsModel(
        otherExecutor,
        "d1-conformance-other",
      ),
      ...
      async insertMigrationPoisonRow() {
        ...
        await createD1RequiredInsightsModel(
          primaryExecutor,
          "d1-conformance-primary",
        ).append(row);
