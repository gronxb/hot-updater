...
const getReport = async (
  executor: D1Executor,
  input: Parameters<RequiredInsightsModel["getReport"]>[0],
...
/** Internal landing point for the five-port contract; intentionally not wired publicly yet. */
export const createD1RequiredInsightsModel = (
  executor: D1Executor,
  databaseNamespace: string,
): RequiredInsightsModel => {
  const implementation = createD1Implementation(executor);
  ...
  return {
    append: (row) => implementation.appendBundleEvent(row),
    ...
    pageInstallations,
    getReport: (input) => getReport(executor, input),
    pageReport: (input) => pageReport(executor, input, databaseNamespace),
  };
};
