  ...
  return createDatabasePlugin(
    "s3Database",
    {
      ...
      async getBundles(options) {
        ...
        if (offset > 0) {
          filteredBundles = filteredBundles.slice(offset);
        }

        if (limit) {
          filteredBundles = filteredBundles.slice(0, limit);
        }

        return filteredBundles.map(
          ({ _updateJsonKey, _oldUpdateJsonKey, ...bundle }) => bundle,
        );
      },
      ...
      async commitBundle({ changedSets }) {
        if (changedSets.size === 0) return;

        const changedBundlesByKey: Record<string, Bundle[]> = {};
        const removalsByKey: Record<string, string[]> = {};

        for (const { operation, data } of changedSets) {
          const bundle = bundles.find((b) => b.id === data.id);
          if (!bundle) continue;

          if (operation === "update" && bundle._oldUpdateJsonKey) {
