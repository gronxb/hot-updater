...
type StandaloneAnalyticsOperations = Pick<
  AnalyticsProvider,
  | "appendBundleEvent"
...
const normalizeActiveInput = (
  input: Parameters<AnalyticsProvider["getActiveInstallationOverview"]>[0],
) => {
  if (
    input.window !== "24h" &&
    input.window !== "7d" &&
...
export const createStandaloneAnalyticsOperations = (
  config: StandaloneRepositoryConfig,
): StandaloneAnalyticsOperations => {
  const routes = {
    appendEvent: () =>
  ...
  return {
    async appendBundleEvent(input) {
      ...
      const response = await http.request(route, {
        ...
        headerPolicy: {
          omit: [SDK_VERSION_HEADER],
          ...(typeof sdkVersion === "string"
            ? { set: { [SDK_VERSION_HEADER]: sdkVersion } }
            : {}),
        },
      });
