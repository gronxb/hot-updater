...
const findXCodeProjectFilename = async (cwd) => {
    try {
...
export const getIOSVersion = async (cwd) => {
    ...
    try {
        ...
        const { stdout } = await execPromise(`xcodebuild -project ${projectPath} -showBuildSettings | grep MARKETING_VERSION`);
...
export const getAndroidVersion = async (cwd) => {
...
export const getDefaultTargetVersion = async (cwd, platform) => {
    switch (platform) {
        case "ios":
