import { handleInfraScaffold, type InfraOptions } from "../infra/scaffold";
...
export const infraBootstrap = (operation: AgentInfraOperation) =>
  [
    ...
    `Providers: ${INIT_PROVIDER_NAMES.join(", ")}. The build value is the selected application integration short name.`,
    "",
    ...
    "Read the returned instructions, COMMON.md, ENVIRONMENT.md, manifest.json and deployment.json. Create missing projects/instances/resources needed for the requested setup with authorized provider tools. For setup follow SETUP.md in order; for upgrade follow the applicable version files and use SETUP.md only for required prerequisites. Verify prerequisites, apply the action, verify the result, then record evidence. Save pendingStep with a stable target before each remote mutation and resolve it before retrying. Run the returned doctor command after configuring deployment inputs; repair fail/blocked checks and rerun it until success is true. After deployment, run doctor --scope infrastructure --infra-dir <scaffold-path> --json with the actual server URL, platform, channel and app-version or fingerprint. Finish common.verify, common.local and common.report in COMMON.md. Require doctor exit code 0 and success:true for the selected scope; recorded verifiedSteps or generated files never replace these checks. Report notChecked items separately.",
