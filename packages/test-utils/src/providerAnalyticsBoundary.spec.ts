...
export const capability = universalComponentDataAdapterCapability;\n`,
  ...
  it.each(["src/database.ts", "iac/index.ts"])(
    "rejects a managed preset import from %s",
    async (relativeFile) => {
      const provider = await createProvider();
      const file = path.join(provider, relativeFile);
  ...
  it("allows the managed preset only in explicitly listed composition files", async () => {
    const provider = await createProvider();
    const runtimeEntry = path.join(provider, "runtime", "index.ts");
    const manifest = path.join(provider, "package.json");
    ...
    await writeFile(
      manifest,
      JSON.stringify({
        dependencies: { "@hot-updater/managed": "workspace:*" },
      }),
    );

    await expect(
      findProviderAnalyticsBoundaryViolations({
        allowedManagedPresetFiles: [runtimeEntry, manifest],
        roots: [provider],
      }),
    ).resolves.toEqual([]);
  });
