...
export type ProviderAnalyticsBoundaryOptions = {
  readonly allowedManagedPresetFiles?: readonly string[];
...
const managedPresetImportPattern =
  /(?:\bfrom\s*|\bimport\s*(?:\(\s*)?|\brequire\s*\(\s*)["']@hot-updater\/managed(?:["'/])/g;
const managedPresetDependencyPattern = /["']@hot-updater\/managed["']\s*:/g;
...
> => {
  ...
  const allowedManagedPresetFileSet = new Set(
    allowedManagedPresetFiles.map((file) => path.resolve(file)),
  );
  ...
  for (const root of roots) {
    ...
    for (const file of await sourceFiles(resolvedRoot, includeTests)) {
      ...
      if (!allowedManagedPresetFileSet.has(file)) {
        const pattern =
          path.basename(file) === "package.json"
            ? managedPresetDependencyPattern
            : managedPresetImportPattern;
        for (const match of contents.matchAll(pattern)) {
          const location = locationAt(contents, match.index);
          violations.push({
            file,
            rule: "Managed preset import",
            ...location,
          });
        }
      }
...
export const assertProviderAnalyticsBoundary = async (
  ...
  throw new Error(
    [
      "Ordinary database providers must not depend on optional feature contracts or import the managed preset outside runtime composition roots:",
      ...violations.map(
