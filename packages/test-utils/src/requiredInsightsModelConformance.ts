...
import type { RequiredInsightsModelOracle } from "./requiredInsightsModelOracle";

export type RequiredInsightsModelConformanceHarness =
  RequiredInsightsModelOracle;

export const REQUIRED_INSIGHTS_CONFORMANCE_MAX_CANDIDATE_READS = 4_096;
...
export const requiredInsightsModelConformanceEvents: readonly BundleEventRow[] =
  [
    {
...
const adversarialBundleMovementEvents: readonly BundleEventRow[] = [
  {
    ...requiredInsightsModelConformanceEvents[0]!,
  ...
  {
    ...requiredInsightsModelConformanceEvents[2]!,
...
const outOfOrderLatestEvents: readonly BundleEventRow[] = [
  {
    ...requiredInsightsModelConformanceEvents[3]!,
...
const appendFixtures = async (
  harness: RequiredInsightsModelConformanceHarness,
): Promise<void> => {
  for (const event of requiredInsightsModelConformanceEvents) {
...
const assertCandidateReadBudget = (
  ...
  expect(declared).toBeLessThanOrEqual(
    REQUIRED_INSIGHTS_CONFORMANCE_MAX_CANDIDATE_READS,
  );
...
/**
 * Runs the provider-neutral five-method contract. The factory must return a
 * fresh namespace pair and test-only durable-job controls for every test.
 */
export const registerRequiredInsightsModelTests = (
  createHarness: () =>
    | RequiredInsightsModelConformanceHarness
    | Promise<RequiredInsightsModelConformanceHarness>,
): void => {
  describe("required Insights model conformance", () => {
    ...
    it("bounds event reads and binds cursors to namespace, scope, and cutoff", async () => {
      ...
      await harness.model.append({
        ...requiredInsightsModelConformanceEvents[1]!,
    ...
    it("uses finite calendar buckets, fixed report order, and zero rows", async () => {
      ...
      const movementBase = requiredInsightsModelConformanceEvents[0]!;
      const activityBase = requiredInsightsModelConformanceEvents[3]!;
