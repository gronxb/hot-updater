...
/** Fingerprints native compatibility inputs owned by a React Native project. */
export async function createReactNativeFingerprint(
  cwd: string,
  ...
  if (paths.size === 0) {
    throw new Error(
      `React Native fingerprint found no meaningful ${options.platform} native inputs.`,
    );
  }
    ...
    [...paths.values()]
      ...
      .map(async (source) => {
        ...
        return {
          ...
          reasons: ["react-native-native-input"],
          hash,
  ...
  digest.update(`react-native:${options.platform}\0`);
