...
describe("automatic notifyAppReady insights", () => {
  beforeEach(() => {
    vi.resetModules();
    vi.unstubAllGlobals();
    vi.useRealTimers();
  ...
  it("sends RECOVERED with directional ids and invokes readiness", async () => {
    ...
    init({ insights: true, client, onNotifyAppReady });
    await vi.runOnlyPendingTimersAsync();
  ...
  it("skips automatic insights when disabled while preserving readiness", async () => {
    stubNotifyFrame();
    const { client, sendInsightsEvent } = createClient();
    const onNotifyAppReady = vi.fn();
    const { init } = await import("./wrap");

    init({ insights: false, client, onNotifyAppReady });
    await vi.runOnlyPendingTimersAsync();
