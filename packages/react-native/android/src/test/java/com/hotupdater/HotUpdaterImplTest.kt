...
class HotUpdaterImplTest {
    ...
    @Test
    fun `updateBundle with valid URL succeeds`() = runTest {
    ...
    @Test
    fun `updateBundle with null URL succeeds (reset)`() = runTest {
        val bundleId = "test-bundle-123"
    ...
    @Test
    fun `updateBundle fails when storage fails`() = runTest {
        val bundleId = "test-bundle-123"
        val fileUrl = "https://example.com/bundle.zip"
    ...
    @Test
    fun `updateBundle invokes progress callback`() = runTest {
        val bundleId = "test-bundle-123"
        val fileUrl = "https://example.com/bundle.zip"
        ...
        val result = impl.updateBundle(bundleId, fileUrl) { progress ->
    ...
    @Test
    fun `different bundle IDs maintain isolation`() = runTest {
...
class HotUpdaterImplIntegrationTest {
    ...
    @Test
    fun `full OTA update workflow with file system isolation`() = runTest {
        ...
        val storage = BundleFileStorageService(
            mockFS,
            mockDownload,
            mockUnzip,
            mockPrefs
    ...
    @Test
    fun `multiple OTA updates maintain version isolation`() = runTest {
        val mockFS = mock<FileSystemService>()
        val mockDownload = mock<DownloadService>()
        val mockUnzip = mock<UnzipService>()
        val mockPrefs = mock<PreferencesService>()
    ...
    @Test
    fun `OTA update correctly extracts and verifies bundle file`() = runTest {
        val mockFS = mock<FileSystemService>()
        val mockDownload = mock<DownloadService>()
        val mockUnzip = mock<UnzipService>()
        val mockPrefs = mock<PreferencesService>()
