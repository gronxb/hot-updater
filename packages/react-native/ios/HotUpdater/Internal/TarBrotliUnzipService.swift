...
class TarBrotliUnzipService: UnzipService {
    ...
    /**
     * Validates if a file is a valid tar.br file.
     * @param atPath Path to the tar.br file
    ...
    /**
     * Unzips a tar.br file to a destination directory with progress tracking.
     * @param file Path to the tar.br file
     * @param destination Directory to extract to
    ...
    func unzip(file: String, to destination: String, progressHandler: @escaping (Double) -> Void) throws {
        ...
        // Extract each entry with progress tracking
        let totalEntries = tarEntries.count
        ...
        NSLog("[TarBrotliUnzipService] Successfully extracted \(extractedCount) entries")
        progressHandler(1.0)
    }
