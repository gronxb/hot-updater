...
import { createInsightsOverview } from "./insights-overview";
...
describe("createInsightsOverview", () => {
  it("aggregates one latest row per tracked installation without leaking identity", () => {
    ...
    // When
    const overview = createInsightsOverview(
      bundles,
      releases,
      channels,
      latestRows,
  ...
  it("sorts equal latest-reported counts by bundle id for every input order", () => {
    ...
    // When
    const forward = createInsightsOverview(bundles, releases, channels, rows);
    const reverse = createInsightsOverview(
      bundles,
      releases,
      channels,
      [...rows].reverse(),
