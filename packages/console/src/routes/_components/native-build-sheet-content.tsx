...
import { useNativeBuildDownloadUrlQuery, useNativeBuildQuery } from "@/lib/api";
import {
  Download,
  HardDrive,
  Hash,
...
export function NativeBuildSheetContent(props: NativeBuildSheetContentProps) {
  const buildQuery = useNativeBuildQuery(props.buildId);
  const downloadUrlQuery = useNativeBuildDownloadUrlQuery(props.buildId);
    ...
    <SheetContent class="w-[400px] sm:w-[540px]">
      <SheetHeader>
        <SheetTitle class="flex items-center gap-2">
          <Show when={buildQuery.data?.platform === "ios"}>
            <AiFillApple size={20} />
          </Show>
          <Show when={buildQuery.data?.platform === "android"}>
      ...
      <div class="space-y-6 py-6">
        <Show when={buildQuery.isLoading}>
          <div class="flex justify-center py-8">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" />
          </div>
          <div class="text-center">
            <p class="text-muted-foreground">Loading native build details...</p>
          </div>
        </Show>

        <Show when={buildQuery.error}>
          <div class="p-4 bg-red-50 rounded-lg">
            <p class="text-sm text-red-700">
              Failed to load native build: {buildQuery.error?.message}
            </p>
          </div>
        </Show>

        <Show
          when={!buildQuery.isLoading && !buildQuery.error && buildQuery.data}
        >
          {/* Basic Information */}
          <div class="space-y-4">
            ...
            <div class="space-y-4">
                ...
                <div class="flex gap-2">
                  ...
                  <Tooltip openDelay={0} closeDelay={0}>
                    <TooltipTrigger class="font-mono text-sm">
                      {buildQuery.data?.id?.slice(0, 8)}
                    </TooltipTrigger>
                    <TooltipContent>
                      <p class="font-mono text-sm">{buildQuery.data?.id}</p>
                    </TooltipContent>
                  </Tooltip>
                </div>
              </div>
              ...
              <div class="space-y-2">
                ...
                <div class="flex items-center gap-2">
                  <Tag size={16} />
                  <span class="font-mono">{buildQuery.data?.channel}</span>
                </div>
              </div>
              ...
              <div class="space-y-2">
                ...
                <div class="flex items-center gap-2">
                  <HardDrive size={16} />
                  <span class="text-sm">
                    {((buildQuery.data?.fileSize || 0) / 1024 / 1024).toFixed(
                      2,
                    )}{" "}
                    MB
                  </span>
              ...
              <div class="space-y-2">
                ...
                <div class="flex gap-2">
                  ...
                  <Tooltip openDelay={0} closeDelay={0}>
                    <TooltipTrigger class="font-mono text-sm">
                      {buildQuery.data?.fingerprintHash?.slice(0, 8)}
                    </TooltipTrigger>
                    <TooltipContent>
                      <p class="font-mono text-sm">
                        {buildQuery.data?.fingerprintHash}
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </div>
          ...
          <Show
            when={
              buildQuery.data?.metadata &&
              Object.keys(buildQuery.data.metadata).length > 0
            }
          >
            <div class="space-y-4">
              ...
              <div class="p-4 bg-gray-50 rounded-lg">
                <pre class="text-sm font-mono overflow-x-auto">
                  {JSON.stringify(buildQuery.data?.metadata, null, 2)}
                </pre>
              </div>
            </div>
            <hr class="border-gray-200" />
            ...
            <div class="space-y-3">
              ...
              <Show
                ...
                fallback={
                  <Show
                    when={
                      !downloadUrlQuery.isLoading && !downloadUrlQuery.error
                    }
                  >
                    <div class="p-4 bg-gray-50 rounded-lg">
              ...
              >
                ...
                <div class="p-3 bg-blue-50 rounded-lg">
                  <p class="text-sm text-blue-700">
                    ...
                    {buildQuery.data?.platform} version{" "}
                    {buildQuery.data?.nativeVersion}.
                  </p>
                </div>
              </Show>
