...
class TarArchiveInputStream(
    ...
    var hasEndMarker: Boolean = false
        private set
    ...
    override fun read(
        ...
        if (bytesRead < 0) throw EOFException("Truncated TAR entry")
        if (bytesRead > 0) {
    ...
    private fun readLongName(size: Long): String {
        require(size in 1..ArchiveLimits.MAX_METADATA_BYTES) { "GNU long name exceeds metadata limit" }
    ...
    private fun readPaxPath(size: Long): String? {
        if (size < 0 || size > ArchiveLimits.MAX_METADATA_BYTES) {
