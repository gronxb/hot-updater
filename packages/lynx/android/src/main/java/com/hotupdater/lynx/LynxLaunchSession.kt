    ...
    fun bind(view: LynxView) {
        ...
        view.addLynxViewClient(object : LynxViewClient() {
            ...
            override fun onReceivedError(error: LynxError) {
                ...
                val jsError = error.errorCode == 201
                val trial = controller.isUnconfirmedBundleTrial()
                android.util.Log.i("HotUpdaterLynx", "engine-error fatal=${error.isFatal} e2eCrash=$e2eCrash jsError=$jsError trial=$trial code=${error.errorCode} message=${error.msg}")
                if (error.isFatal || e2eCrash || (jsError && trial)) {
