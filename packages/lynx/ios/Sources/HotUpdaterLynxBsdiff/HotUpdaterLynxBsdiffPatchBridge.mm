// Adapted from the MIT-licensed Hot Updater RN BSDIFF bridge; see ../../ADAPTATION.md.
#import "HotUpdaterLynxBsdiffPatchBridge.h"
...
NSError *MakePatchError(NSString *message) {
  return [NSError errorWithDomain:@"HotUpdater.Lynx.Bsdiff"
                             code:1
...
extern "C" BOOL HotUpdaterLynxApplyBsdiffPatch(NSString *patchPath,
                                            NSString *basePath,
                                            NSString *outputPath,
                                            uint64_t maximumOutputBytes) {
  NSError *patchError = nil;
  BOOL applied = [HotUpdaterLynxBsdiffPatchBridge applyPatchAtPath:patchPath
                                         toBaseAtPath:basePath
                                        outputAtPath:outputPath
                                  maximumOutputBytes:maximumOutputBytes
                                               error:&patchError];
  if (!applied && patchError != nil) {
    NSLog(@"[HotUpdaterLynx] Failed to apply bsdiff patch: %@",
          patchError.localizedDescription);
  }
  return applied;
}

@implementation HotUpdaterLynxBsdiffPatchBridge
...
+ (BOOL)applyPatchAtPath:(NSString *)patchPath
  ...
  if (fstat(fileno(patchFile), &patchStat) != 0 || !S_ISREG(patchStat.st_mode)) {
    return failMessage(@"Patch input is not a regular file");
  }
  ...
  if (static_cast<uint64_t>(newSize) > maximumOutputBytes) {
  ...
  if (!S_ISREG(baseStat.st_mode)) {
    return failMessage(@"Patch base is not a regular file");
  }
  ...
  while (outputPos < newSize) {
    ...
    if (!ReadBzipExactly(bz2, controlBytes, sizeof(controlBytes), &streamEnded, error)) {
      cleanup();
      return NO;
    }
    ...
    if (addLen < 0 || copyLen < 0 || (addLen == 0 && copyLen == 0)) {
      return failMessage(@"Invalid non-progressing add/copy length in control block");
    }
    ...
    while (remainingAdd > 0) {
      ...
      if (!ReadBzipExactly(bz2, diffBuffer.data(), chunkSize, &streamEnded, error) ||
  ...
  if (!streamEnded) {
    ...
    bz2err = BZ_OK;
    int trailingRead = BZ2_bzRead(&bz2err, bz2, &trailingByte, 1);
    if (trailingRead != 0 || bz2err != BZ_STREAM_END) {
      return failMessage(@"Trailing or malformed ENDSLEY/BSDIFF43 content");
    }
  }
  void *unusedBytes = nullptr;
  int unusedCount = 0;
  BZ2_bzReadGetUnused(&bz2err, bz2, &unusedBytes, &unusedCount);
  if (bz2err != BZ_OK || unusedCount != 0 || fgetc(patchFile) != EOF) {
    return failMessage(@"Trailing ENDSLEY/BSDIFF43 content");
  }
