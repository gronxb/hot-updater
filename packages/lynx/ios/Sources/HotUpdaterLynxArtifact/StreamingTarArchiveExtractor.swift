...
public enum StreamingTarArchiveExtractor {
    ...
    static func extractCompressedTar(
        ...
        try withTemporaryTarFile(directory: strict ? URL(fileURLWithPath: destination).deletingLastPathComponent() : nil) { temporaryTarURL in
            switch algorithm {
            case .gzip:
    ...
    private static func withTemporaryTarFile<T>(
        ...
        let temporaryTarURL = (directory ?? FileManager.default.temporaryDirectory)
