...
enum ZipArchiveExtractor {
    ...
    static func extract(file: String, to destination: String, strict: Bool = false, progressHandler: @escaping (Double) -> Void) throws {
        ...
        let entries = try readCentralDirectoryEntries(from: handle, fileSize: fileSize, strict: strict)
        ...
        for entry in entries {
            if strict {
                try entryGuard.admit(entry.path, size: entry.uncompressedSize, directory: entry.isDirectory, link: entry.isSymbolicLink)
                guard entry.localHeaderOffset < fileSize, entry.compressedSize <= fileSize - entry.localHeaderOffset else { throw ArchiveLimits.reject("ZIP entry range exceeds archive") }
            }
            try extractEntry(entry, from: handle, to: destinationURL.standardizedFileURL.path, strict: strict)
    ...
    private static func readCentralDirectoryEntries(
        ...
        if strict && Int(endRecord.totalEntries) > ArchiveLimits.entries { throw ArchiveLimits.reject("ZIP entry count limit exceeded") }
        ...
        for _ in 0..<endRecord.totalEntries {
            ...
            if strict && (fileNameLength == 0 || fileNameLength > 1024) { throw ArchiveLimits.reject("ZIP path length limit exceeded") }
            ...
            if strict, let mode, ![UInt16(0o100000), directoryMode].contains(mode) { throw ArchiveLimits.reject("ZIP special file rejected") }
    ...
    private static func extractEntry(
        ...
        guard let relativePath = ArchiveExtractionUtilities.normalizedRelativePath(from: strict && entry.isDirectory && entry.path.hasSuffix("/") ? String(entry.path.dropLast()) : entry.path) else {
            return
        }
        ...
        let localName = try ArchiveExtractionUtilities.readExactly(from: handle, count: Int(fileNameLength))
        if strict {
            guard String(data: localName, encoding: .utf8) == entry.path,
                  localHeader.archiveUInt16LE(at: 8) == entry.compressionMethod,
                  localHeader.archiveUInt16LE(at: 6) == entry.flags else { throw ArchiveLimits.reject("Conflicting ZIP local header") }
        }
        ...
        case storedMethod:
            extractionResult = try extractStoredEntry(
                ...
                to: outputHandle,
                limit: strict ? entry.uncompressedSize : nil
            )
        case deflatedMethod:
    ...
    private static func extractStoredEntry(
        ...
        while remainingBytes > 0 {
            ...
            if let limit {
                guard totalWritten <= limit, UInt64(chunk.count) <= limit - totalWritten else { throw ArchiveLimits.reject("ZIP output exceeds declared size") }
                try ArchiveLimits.checkOutput(outputHandle, adding: chunk.count)
            }
            ...
            try chunk.withUnsafeBytes { rawBuffer in
                ...
                repeat {
                    ...
                    if producedBytes > 0 {
                        ...
                        if let limit {
                            guard totalWritten <= limit, UInt64(producedBytes) <= limit - totalWritten else { throw ArchiveLimits.reject("ZIP output exceeds declared size") }
                            try ArchiveLimits.checkOutput(outputHandle, adding: producedBytes)
                        }
