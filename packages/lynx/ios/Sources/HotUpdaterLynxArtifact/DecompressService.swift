...
public class DecompressService {
     ...
     */
    public func unzip(file: String, to destination: String, progressHandler: @escaping (Double) -> Void) throws {
     ...
     * @throws Error if decompression fails or no valid strategy found
     */
    public func unzip(file: String, to destination: String) throws {
     ...
     */
    public func isValid(file: String) -> Bool {
        for strategy in signatureStrategies {
            if strategy.isValid(file: file) {
                return true
            }
