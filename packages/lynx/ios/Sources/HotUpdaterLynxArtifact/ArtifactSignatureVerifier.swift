 ...
 */
public class ArtifactSignatureVerifier {
     ...
     */
    public static func verifyBundle(fileURL: URL, fileHash: String?, publicKeyPEM: String?) -> Result<Void, SignatureVerificationError> {
        let signingEnabled = publicKeyPEM != nil
        ...
        if isSignedFormat(hash) {
            ...
            return verifySignature(fileURL: fileURL, signatureBase64: signature, publicKeyPEM: publicKeyPEM)
        } else {
     ...
     * @return Result indicating verification success or failure with error
     */
    public static func verifySignature(fileURL: URL, signatureBase64: String, publicKeyPEM: String?) -> Result<Void, SignatureVerificationError> {
        NSLog("[SignatureVerifier] Verifying signature for file: \(fileURL.lastPathComponent)")

        // Get public key from config
        guard let publicKeyPEM else {
     ...
     */
    public static func verifyHashSignature(fileHash: String, signatureBase64: String, publicKeyPEM: String?) -> Result<Void, SignatureVerificationError> {
        guard let publicKeyPEM else {
            NSLog("[SignatureVerifier] Cannot verify signature: public key not configured in Info.plist")
            return .failure(.publicKeyNotConfigured)
        }
