...
installPrecompiledWasm(loadBunWasmModule());

function loadBunWasmModule(): WebAssembly.Module {
  try {
    return new WebAssembly.Module(readFileSync(HDIFF_WASM_URL));
  } catch {
    throw new HdiffError(
      "PATCH_FAILED",
      `Failed to load hdiff.wasm for Bun runtime: ${HDIFF_WASM_URL.toString()}`,
    );
  }
}
