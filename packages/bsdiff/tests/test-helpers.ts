...
export async function readFixtureHbc(name: "one" | "two"): Promise<Uint8Array> {
  const fixturePath = path.resolve(ROOT, "fixture", name, "index.ios.bundle.hbc");
  const bytes = await fs.readFile(fixturePath);
  return new Uint8Array(bytes);
}

export async function applyBspatch(base: Uint8Array, patch: Uint8Array): Promise<Uint8Array> {
...
export function withFileLength(input: Uint8Array, fileLength: number): Uint8Array {
  const copy = new Uint8Array(input);
  const view = new DataView(copy.buffer, copy.byteOffset, copy.byteLength);
