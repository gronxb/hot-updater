...
/**
 * Integration test context for HTTP-based integration testing.
 */
...
/**
 * Helper function to fetch update info from the server via HTTP.
 * Makes a POST request with GetBundlesArgs and returns UpdateInfo response.
 *
 * @param context - Integration test context with server URL and headers
 * @param args - GetBundlesArgs parameters for the update check
 * @returns UpdateInfo response or null if no update available
 * @throws Error if the HTTP request fails or returns non-2xx/404 status
 */
export async function fetchUpdateInfo(
  ...
  // 404 typically means no update available
  ...
  // Empty response body indicates no update
...
/**
 * Sets up the complete integration test suite for getUpdateInfo via HTTP.
 * This test suite makes actual HTTP requests to a running server and validates responses.
 *
 * @param context - Integration test context with server URL and database management functions
 *
 * @example
 * ```typescript
...
export const setupGetUpdateInfoIntegrationTestSuite = (
  ...
  describe("app version strategy", () => {
    it("applies an update when a '*' bundle is available", async () => {
      ...
      await context.seedBundles(bundles);
