...
const createHarness = async (): Promise<InsightsModelConformanceHarness> => {
  ...
  const primaryNamespace = randomUUID();
  const otherNamespace = randomUUID();
  const initialPrimary = await connect(primaryName);
  const initialOther = await connect(otherName);
  await prepareNamespace(initialPrimary, primaryNamespace);
  await prepareNamespace(initialOther, otherNamespace);
  ...
  const createFacade = async (
    ...
    const primaryModel = instrumentModel(
      createMongoInsightsModel(primary, primaryNamespace),
      primaryMeter,
      jobKinds,
    );
    const otherModel = instrumentModel(
      createMongoInsightsModel(other, otherNamespace),
      otherMeter,
      jobKinds,
    );
    const runJobStep = async (
      ...
      const maintenance = createMongoInsightsModelMaintenance(
        workerClient,
        workerClient === other ? otherNamespace : primaryNamespace,
      );
