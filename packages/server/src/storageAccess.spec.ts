...
describe("createStorageAccess", () => {
  ...
  it("reads text through a matching runtime storage plugin before direct HTTP fetch", async () => {
    ...
    const { readStorageText } = createStorageAccess(
      createRuntimeStorageRecords([storagePlugin]),
    );
  ...
  it("uses the implementation selected by the configured factory", async () => {
    const readText = vi.fn(async () => "implementation text");
    const factory = vi.fn(() => ({
    ...
    const exposedReadText = vi.spyOn(
      storagePlugin.profiles.runtime,
      "readText",
    );

    const records = createRuntimeStorageRecords([storagePlugin]);
    const { readStorageText } = createStorageAccess(records);
    expect(factory).not.toHaveBeenCalled();

    await expect(
      readStorageText("https://assets.example.com/manifest.json"),
    ).resolves.toBe("implementation text");
    expect(factory).toHaveBeenCalledOnce();
    expect(readText).toHaveBeenCalledOnce();
    expect(exposedReadText).not.toHaveBeenCalled();
  });
