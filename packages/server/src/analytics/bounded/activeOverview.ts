...
type ActiveWindowDefinition =
  (typeof activeWindowDefinitions)[ActiveInstallationWindow];

export function getActiveWindowDefinition(
...
function isNewer(
  candidate: BundleEventPersistenceRow,
  current: BundleEventPersistenceRow,
): boolean {
  return (
...
export function collectActiveInstallationOverview(
  request: ActiveOverviewRequest,
): ActiveInstallationOverview {
  const definition = getActiveWindowDefinition(request.window);
  const windowStartMs =
    request.asOfMs - definition.bucketCount * definition.bucketSizeMs;
  ...
  for (const row of rows) {
    ...
    if (current === undefined || isNewer(row, current)) {
      latestByInstall.set(row.install_id, row);
    }
  }
  const selectedRows = [...latestByInstall.values()].filter(
    (row) => request.userId === undefined || row.user_id === request.userId,
  );
  ...
  const latestByBucket = Array.from(
    ...
    () => new Map<string, BundleEventPersistenceRow>(),
  );
  for (const row of rows) {
