...
describe("createAnalyticsProvider", () => {
  ...
  it("counts distinct installations across cursor pages", async () => {
    ...
    const provider = createAnalyticsProvider(inMemoryPersistence(rows));
  ...
  it("continues scanning when persistence caps pages below the requested limit", async () => {
    ...
    const provider = createAnalyticsProvider(inMemoryPersistence(rows, 17));
  ...
  it("counts an installation whose current bundle is reported by UNCHANGED", async () => {
    ...
    const provider = createAnalyticsProvider(inMemoryPersistence([unchanged]));
