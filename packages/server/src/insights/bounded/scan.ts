...
export const INSIGHTS_SCAN_MAX_ROWS = 50_000;
export const INSIGHTS_MATERIALIZATION_LIMIT = INSIGHTS_SCAN_MAX_ROWS + 1;
export const INSIGHTS_SCAN_PAGE_SIZE = 1_000;
const INSIGHTS_LOWER_BOUND_ID = "00000000-0000-0000-0000-000000000000";

export type InsightsScanScope = {
  readonly persistence: InsightsPersistence;
...
export const materializeEventRows = async (
  ...
  let after =
    scope.lowerBoundMs === undefined
      ? undefined
      : { receivedAtMs: scope.lowerBoundMs, id: INSIGHTS_LOWER_BOUND_ID };
  while (rows.length < INSIGHTS_MATERIALIZATION_LIMIT) {
    const limit = Math.min(
      INSIGHTS_SCAN_PAGE_SIZE,
      INSIGHTS_MATERIALIZATION_LIMIT - rows.length,
    );
    ...
    for (const row of page) {
      if (
        ...
        throw new InsightsPersistenceOrderError();
      }
      seenIds.add(row.id);
      rows.push(row);
      previous = row;
    ...
    if (rows.length > INSIGHTS_SCAN_MAX_ROWS) {
      throw new InsightsScanLimitExceededError(INSIGHTS_SCAN_MAX_ROWS);
    }
  }
  return rows;
};

export class InsightsPersistenceOrderError extends Error {
  readonly name = "InsightsPersistenceOrderError";
...
export const getWindowRange = (
  window: Exclude<BundleEventInsightsWindow, "all">,
  now: number,
