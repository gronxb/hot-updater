...
import {
  ...
  assertInsightsResult,
  createValidatedInsightsModel,
} from "./provider";
import { createInsightsRouteHandlers } from "./routes";
...
describe("Insights server boundary", () => {
  ...
  it("accepts only the legal states for each required read operation", () => {
    ...
    for (const [value, kind, limit] of [
      ...
      expect(() => assertInsightsResult(value, kind, limit)).not.toThrow();
    }
    ...
    for (const [value, kind, limit] of [
      ...
      expect(() => assertInsightsResult(value, kind, limit)).toThrow();
    }
  });

  it("binds cutoffs, publications, sections and report IDs to the input", () => {
    expect(() =>
      assertInsightsOperationResult(eventPage(), {
        kind: "events",
        input: {
          selector: { kind: "all" },
          beforeReceivedAtMs: 1_000,
    ...
    expect(() =>
      assertInsightsOperationResult(publishedInstallationPage, {
        kind: "installations",
        input: {
          kind: "contains",
          query: "user",
    ...
    expect(() =>
      assertInsightsOperationResult(reportPage, {
        kind: "report-page",
        input: {
          publicationId: publication.id,
          section: "activeSeries",
    ...
    expect(() =>
      assertInsightsOperationResult(report, {
        kind: "report",
        input: {
  ...
  it("accepts cursor-pinned expiry and surfaces wrong-query cursors as invalid", async () => {
    ...
    const provider = createValidatedInsightsModel(model, beforeOperation);
    await expect(
  ...
  it("checks readiness before each Insights model operation", async () => {
    const model = createInsightsModel();
    const beforeOperation = vi.fn().mockResolvedValue(undefined);
    const provider = createValidatedInsightsModel(model, beforeOperation);
    const row = {
