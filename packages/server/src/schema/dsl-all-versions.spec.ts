...
import { bundlesV036, v0_36_0 } from "./v0_36_0";
...
describe("versioned schema DSL", () => {
  it("uses the local functional DSL for every versioned schema table", () => {
    ...
    expect(v0_38_0.dsl).toBe("schema");
    ...
    expect(bundlesV038.dsl).toBe("table");
    expect(channelsV038.dsl).toBe("table");
    expect(createSettingsTable("0.31.0").dsl).toBe("table");

    for (const schema of [v0_21_0, v0_29_0, v0_31_0, v0_36_0, v0_38_0]) {
  ...
  it("ends the official-domain registry at v0.38", () => {
